...
function resolveAssistMeRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.ASSISTME_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set ASSISTME_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAssistMeRoot(), "dist", "extensionAPI.js");
