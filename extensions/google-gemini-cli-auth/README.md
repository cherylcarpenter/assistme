# Google Gemini CLI Auth (AssistMe plugin)
...
```bash
assistme plugins enable google-gemini-cli-auth
```
...
```bash
assistme models auth login --provider google-gemini-cli --set-default
```
...
- `ASSISTME_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `ASSISTME_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
