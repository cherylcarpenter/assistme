...
function resolveGatewayPort(cfg: AssistMePluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.ASSISTME_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: AssistMePluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: AssistMePluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the AssistMe iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the AssistMe iOS app:",
            "",
            "```",
            qrAscii,
            "```",
