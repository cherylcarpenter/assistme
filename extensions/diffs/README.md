...
Read-only diff viewer plugin for **AssistMe** agents.
...
Set plugin-wide defaults in `~/.assistme/assistme.json`:
...
Before:
AssistMe supports plugins.

After:
AssistMe supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/assistme-diffs`).
