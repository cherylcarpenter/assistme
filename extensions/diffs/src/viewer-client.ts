...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-assistme-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-assistme-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.assistmeDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.assistmeDiffsError = "true";
