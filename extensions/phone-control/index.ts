...
import type { AssistMePluginApi, AssistMePluginService } from "assistme/plugin-sdk";
...
function normalizeDenyList(cfg: AssistMePluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AssistMePluginApi["config"]): string[] {
...
export default function register(api: AssistMePluginApi) {
  ...
  const timerService: AssistMePluginService = {
