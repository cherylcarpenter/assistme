...
`assistme.invoke --each`) without adding new AssistMe code per workflow.
...
This extension depends on AssistMe internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AssistMe extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.assistme/extensions` as a standalone plugin directory.
