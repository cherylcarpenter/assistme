...
metadata: { "assistme": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == AssistMe `sessions_spawn`
- **File I/O** == AssistMe `read`/`write`
- **Remote fetch** == AssistMe `web_fetch` (or `exec` with curl when POST is required)
