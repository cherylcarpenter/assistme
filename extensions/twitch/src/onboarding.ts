...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: ASSISTME_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: AssistMeConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var ASSISTME_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.ASSISTME_TWITCH_ACCESS_TOKEN?.trim();
