/**
 * Twitch channel plugin for AssistMe.
 *
...
import { buildChannelConfigSchema } from "assistme/plugin-sdk";
 ...
 * for AssistMe. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: AssistMeConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: AssistMeConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: AssistMeConfig): boolean => {
