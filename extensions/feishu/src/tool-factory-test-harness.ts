import type { AnyAgentTool, AssistMePluginApi } from "assistme/plugin-sdk";
...
export function createToolFactoryHarness(cfg: AssistMePluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<AssistMePluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
