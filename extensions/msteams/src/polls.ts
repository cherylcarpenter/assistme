...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["assistmePollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["assistme", "pollId"]) ??
    readNestedString(value, ["assistme", "poll", "id"]) ??
    readNestedString(value, ["data", "assistmePollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "assistme", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          assistmePollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "assistme poll vote",
          displayText: "Vote recorded",
          value: { assistmePollId: pollId, pollId },
        },
      },
    ],
  };
