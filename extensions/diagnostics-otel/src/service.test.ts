...
vi.mock("assistme/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("assistme/plugin-sdk")>("assistme/plugin-sdk");
  return {
    ...actual,
...
import type { AssistMePluginServiceContext } from "assistme/plugin-sdk";
import { emitDiagnosticEvent } from "assistme/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/assistme-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): AssistMePluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("assistme.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("assistme.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("assistme.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("assistme.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("assistme.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("assistme.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("assistme.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("assistme.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("assistme.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("assistme.webhook.processed");
    expect(spanNames).toContain("assistme.message.processed");
    expect(spanNames).toContain("assistme.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["assistme.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("assistme.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "assistme.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["assistme.reason"]).toBe("string");
    expect(String(attrs?.["assistme.reason"])).not.toContain(
