...
import type { PluginRuntime } from "assistme/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ASSISTME_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-nostr-"));
  process.env.ASSISTME_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.ASSISTME_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".assistme");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.ASSISTME_STATE_DIR = previous;
    }
