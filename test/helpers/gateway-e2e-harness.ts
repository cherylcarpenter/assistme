...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `assistme-e2e-${name}-`));
  const configDir = path.join(homeDir, ".assistme");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ASSISTME_SKIP_PROVIDERS: "1",
          ASSISTME_SKIP_GMAIL_WATCHER: "1",
          ASSISTME_SKIP_CRON: "1",
          ASSISTME_SKIP_BROWSER_CONTROL_SERVER: "1",
          ASSISTME_SKIP_CANVAS_HOST: "1",
          ASSISTME_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
