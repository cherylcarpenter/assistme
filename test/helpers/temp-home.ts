...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ASSISTME_HOME.
  delete process.env.ASSISTME_HOME;
  process.env.ASSISTME_STATE_DIR = path.join(base, ".assistme");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "assistme-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".assistme", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
