...
If you believe you've found a security issue in AssistMe, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [assistme/assistme](https://github.com/assistme/assistme)
- **macOS desktop app** — [assistme/assistme](https://github.com/assistme/assistme) (apps/macos)
- **iOS app** — [assistme/assistme](https://github.com/assistme/assistme) (apps/ios)
- **Android app** — [assistme/assistme](https://github.com/assistme/assistme) (apps/android)
- **ClawHub** — [assistme/clawhub](https://github.com/assistme/clawhub)
- **Trust and threat model** — [assistme/trust](https://github.com/assistme/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@assistme.ai](mailto:security@assistme.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.assistme.ai).
...
- Tested version details (AssistMe version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to AssistMe's documented trust boundaries.
- For exposed-secret reports: proof the credential is AssistMe-owned (or grants access to AssistMe-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at AssistMe. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

AssistMe is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
AssistMe does **not** model one gateway as a multi-tenant, adversarial user boundary.

- Authenticated Gateway callers are treated as trusted operators for that gateway instance.
- Session identifiers (`sessionKey`, session IDs, labels) are routing controls, not per-user authorization boundaries.
- If one operator can view data from another operator on the same gateway, that is expected in this trust model.
- AssistMe can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need AssistMe, use one VPS (or host/OS user boundary) per user.
...
- This is expected in AssistMe's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of AssistMe's trusted computing base for a gateway.
...
- Using AssistMe in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.assistme`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not AssistMe-owned and not granting access to AssistMe-operated infrastructure/services) without demonstrated AssistMe impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

AssistMe security guidance assumes:

- The host where AssistMe runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.assistme` state/config (including `assistme.json`) is effectively a trusted operator.
...
AssistMe's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
AssistMe separates routing from execution, but both remain inside the same operator trust boundary:
...
- Plugins can execute with the same OS privileges as the AssistMe process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

AssistMe uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/assistme` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/assistme` (or `assistme-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the AssistMe-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use AssistMe temp helpers (`resolvePreferredAssistMeTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-assistme-dir.ts`
...
For threat model + hardening guidance (including `assistme security audit --deep` and `--fix`), see:

- `https://docs.assistme.ai/gateway/security`
...
AssistMe's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `assistme gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - AssistMe keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `assistme security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__assistme__/canvas/`, `/__assistme__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__assistme__/canvas/`, `/__assistme__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

AssistMe requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running AssistMe in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v assistme-data:/app/data \
  assistme/assistme:latest
```
