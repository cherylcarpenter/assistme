...
export async function handleWhatsAppStart(host: AssistMeApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: AssistMeApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: AssistMeApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: AssistMeApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: AssistMeApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: AssistMeApp): string {
...
function resolveGatewayHttpAuthHeader(host: AssistMeApp): string | null {
...
function buildGatewayHttpHeaders(host: AssistMeApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: AssistMeApp) {
...
export function handleNostrProfileToggleAdvanced(host: AssistMeApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: AssistMeApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: AssistMeApp) {
  const state = host.nostrProfileFormState;
