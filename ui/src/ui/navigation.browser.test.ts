...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/assistme/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/assistme");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/assistme/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ASSISTME_CONTROL_UI_BASE_PATH__ = "/assistme";
    const app = mountApp("/assistme/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/assistme");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/assistme/sessions");
  });
