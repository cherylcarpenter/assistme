...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.assistme/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.assistme/node.json` (node id + pairing token).
...
Path: `~/.assistme/exec-approvals.json`
...
- Unix socket at `~/.assistme/exec-approvals.sock` (0600).
