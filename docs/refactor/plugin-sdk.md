...
- Publish as `assistme/plugin-sdk` (or export from core under `assistme/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `AssistMePluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AssistMeConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AssistMeConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AssistMeConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AssistMeConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AssistMeConfig): string;
  };
};
```
...
- Introduce `assistme/plugin-sdk`.
- Add `api.runtime` to `AssistMePluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `assistmeRuntime: ">=2026.2.0"`).
