...
summary: 设置指南：在保持最新的同时保持你的 AssistMe 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.assistme/workspace`（工作区）+ `~/.assistme/assistme.json`（配置）。
...
- **配置：** `~/.assistme/assistme.json`（JSON/JSON5 格式）
- **工作区：** `~/.assistme/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm assistme setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **AssistMe.app**（菜单栏）。
...
- 运行 `assistme setup`，然后 `assistme channels login`，然后手动启动 Gateway 网关（`assistme gateway`）。
...
- **配对允许列表**：`~/.assistme/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.assistme/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.assistme/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.assistme/workspace` 和 `~/.assistme/` 保持为"你的东西"；不要将个人提示/配置放入 `assistme` 仓库。
