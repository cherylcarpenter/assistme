...
- `~/Library/Application Support/AssistMe/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `assistme-canvas://<session>/<path>`

示例：

- `assistme-canvas://main/` → `<canvasRoot>/main/index.html`
- `assistme-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `assistme-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
assistme nodes canvas present --node <id>
assistme nodes canvas navigate --node <id> --url "/"
assistme nodes canvas eval --node <id> --js "document.title"
assistme nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__assistme__/a2ui/
```
...
assistme nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
assistme nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "assistme://agent?message=Review%20this%20design";
```
