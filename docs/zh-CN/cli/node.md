...
summary: "`assistme node` 的 CLI 参考（无头节点主机）"
title: node
x-i18n:
...
```bash
assistme node install --host <gateway-host> --port 18789
```
...
```bash
assistme node status
assistme node stop
assistme node restart
assistme node uninstall
```

使用 `assistme node run` 运行前台节点主机（无服务）。
...
- `assistme approvals --node <id|name|ip>`（从 Gateway 网关编辑）
