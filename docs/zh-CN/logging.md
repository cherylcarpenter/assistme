...
你可以在 `~/.assistme/assistme.json` 中覆盖此设置：
...
所有日志配置都在 `~/.assistme/assistme.json` 的 `logging` 下。
...
- AssistMe 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `assistme plugins enable diagnostics-otel` 启用插件。
...
- `assistme.tokens`（计数器，属性：`assistme.token`、`assistme.channel`、`assistme.provider`、`assistme.model`）
- `assistme.cost.usd`（计数器，属性：`assistme.channel`、`assistme.provider`、`assistme.model`）
- `assistme.run.duration_ms`（直方图，属性：`assistme.channel`、`assistme.provider`、`assistme.model`）
- `assistme.context.tokens`（直方图，属性：`assistme.context`、`assistme.channel`、`assistme.provider`、`assistme.model`）

消息流：

- `assistme.webhook.received`（计数器，属性：`assistme.channel`、`assistme.webhook`）
- `assistme.webhook.error`（计数器，属性：`assistme.channel`、`assistme.webhook`）
- `assistme.webhook.duration_ms`（直方图，属性：`assistme.channel`、`assistme.webhook`）
- `assistme.message.queued`（计数器，属性：`assistme.channel`、`assistme.source`）
- `assistme.message.processed`（计数器，属性：`assistme.channel`、`assistme.outcome`）
- `assistme.message.duration_ms`（直方图，属性：`assistme.channel`、`assistme.outcome`）

队列 + 会话：

- `assistme.queue.lane.enqueue`（计数器，属性：`assistme.lane`）
- `assistme.queue.lane.dequeue`（计数器，属性：`assistme.lane`）
- `assistme.queue.depth`（直方图，属性：`assistme.lane` 或 `assistme.channel=heartbeat`）
- `assistme.queue.wait_ms`（直方图，属性：`assistme.lane`）
- `assistme.session.state`（计数器，属性：`assistme.state`、`assistme.reason`）
- `assistme.session.stuck`（计数器，属性：`assistme.state`）
- `assistme.session.stuck_age_ms`（直方图，属性：`assistme.state`）
- `assistme.run.attempt`（计数器，属性：`assistme.attempt`）
...
- `assistme.webhook.error`
  - `assistme.channel`、`assistme.webhook`、`assistme.chatId`、`assistme.error`
- `assistme.message.processed`
  - `assistme.channel`、`assistme.outcome`、`assistme.chatId`、`assistme.messageId`、`assistme.sessionKey`、`assistme.sessionId`、`assistme.reason`
- `assistme.session.stuck`
  - `assistme.state`、`assistme.ageMs`、`assistme.queueDepth`、`assistme.sessionKey`、`assistme.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `assistme doctor`。
