...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AssistMe
...
Running AssistMe via Node with `tsx` fails at startup with:

```
[assistme] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `assistme status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch assistme.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node assistme.mjs status` works on Node 25.
