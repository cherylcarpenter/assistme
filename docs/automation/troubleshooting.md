...
assistme system heartbeat last
assistme logs --follow
assistme config get agents.defaults.heartbeat
assistme channels status --probe
```
...
```bash
assistme config get agents.defaults.heartbeat.activeHours
assistme config get agents.defaults.heartbeat.activeHours.timezone
assistme config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
assistme cron list
assistme logs --follow
```
