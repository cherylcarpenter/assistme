...
- Jobs persist under `~/.assistme/cron/` so restarts don’t lose schedules.
...
assistme cron list
assistme cron run <job-id>
assistme cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.assistme/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `assistme cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, AssistMe applies a
...
If `delivery` is omitted for isolated jobs, AssistMe defaults to `announce`.
...
- Job store: `~/.assistme/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.assistme/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, AssistMe classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, AssistMe archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `assistme cron runs` and adjust retention before logs become large
...
assistme cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
assistme cron edit <jobId> --agent ops
assistme cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
assistme cron run <jobId>
assistme cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
assistme cron edit <jobId> \
...
```bash
assistme cron edit <jobId> --exact
```
...
```bash
assistme system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`assistme system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `ASSISTME_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- AssistMe applies exponential retry backoff for recurring jobs after consecutive errors:
