...
```bash
assistme tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `assistme tui --deliver`
...
- Local shell commands receive `ASSISTME_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `assistme logs --follow`.
- Confirm the agent can run: `assistme status` and `assistme models status`.
...
- No agents in picker: check `assistme agents list` and your routing config.
