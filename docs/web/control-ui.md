...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/assistme`)
...
If the page fails to load, start the Gateway first: `assistme gateway`.
...
you revoke it with `assistme devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.assistme/assistme.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop assistme`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. AssistMe
...
AssistMe **blocks** Control UI connections without device identity.
