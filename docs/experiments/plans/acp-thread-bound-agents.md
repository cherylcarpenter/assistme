...
This plan defines how AssistMe should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- AssistMe core owns ACP control plane concerns
...
AssistMe should not reimplement ACP transport internals in core.
AssistMe should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in AssistMe, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under AssistMe state dir
...
- ACP event stream is normalized to AssistMe reply chunks
...
For the first production backend (`extensions/acpx`), AssistMe and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses AssistMe `sessionKey`)
...
- AssistMe consumes ndjson events from `acpx --format json --json-strict`
