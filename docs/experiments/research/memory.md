...
read_when:
  - Designing workspace memory (~/.assistme/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep AssistMe integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.assistme/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.assistme/workspace` as canonical human-readable memory.
...
```
~/.assistme/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in AssistMe**, but keep a separable core library.

### Why integrate into AssistMe?

- AssistMe already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `assistme memory recall "…" --k 25 --since 30d`
  - `assistme memory reflect --since 7d`
...
Pragmatic take for `~/.assistme/workspace`:
