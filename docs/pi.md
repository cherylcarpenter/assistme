...
summary: "Architecture of AssistMe's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in AssistMe
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how AssistMe integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

AssistMe uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, AssistMe directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in AssistMe's local TUI mode)                                             |
...
├── pi-tools.ts                    # createAssistMeCodingTools()
...
├── assistme-tools.ts              # AssistMe-specific tools
...
Image injection is prompt-local: AssistMe loads image refs from the current prompt and
...
2. **Custom Replacements**: AssistMe replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **AssistMe Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
AssistMe wraps this with `guardSessionManager()` for tool result safety.
...
AssistMe maintains an auth profile store with multiple API keys per provider:
...
AssistMe loads custom pi extensions for specialized behavior:
...
AssistMe also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | AssistMe Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom AssistMe tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.assistme/agents/<agentId>/sessions/` (or `$ASSISTME_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `ASSISTME_LIVE_TEST=1`)
