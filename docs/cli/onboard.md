---
summary: "CLI reference for `assistme onboard` (interactive onboarding wizard)"
read_when:
...
```bash
assistme onboard
assistme onboard --flow quickstart
assistme onboard --flow manual
assistme onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`ASSISTME_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `assistme dashboard` (Control UI, no channel setup).
