---
summary: "CLI reference for `assistme secrets` (reload, audit, configure, apply)"
read_when:
...
Use `assistme secrets` to migrate credentials from plaintext to SecretRefs and keep the active secrets runtime healthy.
...
```bash
assistme secrets audit
assistme secrets audit --check
assistme secrets audit --json
```
...
```bash
assistme secrets configure
assistme secrets configure --plan-out /tmp/assistme-secrets-plan.json
assistme secrets configure --apply --yes
assistme secrets configure --providers-only
assistme secrets configure --skip-provider-setup
assistme secrets configure --json
```
...
- `configure` targets secret-bearing fields in `assistme.json`.
...
assistme secrets apply --from /tmp/assistme-secrets-plan.json
assistme secrets apply --from /tmp/assistme-secrets-plan.json --dry-run
assistme secrets apply --from /tmp/assistme-secrets-plan.json --json
```
...
- `assistme.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.assistme/.env` known secret keys whose values were migrated
