...
read_when:
  - Running AssistMe behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of AssistMe
...
- You run AssistMe behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. AssistMe checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. AssistMe extracts the user identity from the configured header
...
- AssistMe can stay on loopback HTTP behind the proxy.
...
If AssistMe itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://assistme.example.com
    to: http://assistme-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
assistme.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy assistme:18789 {
...
location / {
    ...
    proxy_pass http://assistme:18789;
...
`assistme security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update AssistMe config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `assistme security audit` and review findings
