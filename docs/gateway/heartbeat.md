...
- During heartbeat runs, AssistMe treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `assistme sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, AssistMe skips the heartbeat run entirely (no model call).
...
```bash
assistme system event --text "Check for urgent follow-ups" --mode now
```
