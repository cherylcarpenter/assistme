...
description: "Complete field-by-field reference for ~/.assistme/assistme.json"
summary: "Complete reference for every AssistMe config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.assistme/assistme.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — AssistMe uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `assistme doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["assistme-dm"] },
      guilds: {
        "123456789012345678": {
...
- AssistMe additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `assistme plugins install @assistme/mattermost`.
...
AssistMe spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `assistme channels add` (or channel onboarding) while still on a single-account top-level channel config, AssistMe moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `assistme doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@assistme"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `assistme.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, AssistMe auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, AssistMe assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "assistme-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.assistme/sandboxes`
...
noVNC observer access uses VNC auth by default and AssistMe emits a short-lived token URL that serves a local bootstrap page; noVNC password is passed via URL fragment (instead of URL query).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `assistme-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `ASSISTME_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `ASSISTME_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `ASSISTME_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.assistme/workspace",
        agentDir: "~/.assistme/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@assistme"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, AssistMe starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:assistme`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.assistme/attachments/<uuid>/` with a `.manifest.json`.
...
AssistMe uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.assistme/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `ASSISTME_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `assistme onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `assistme onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `assistme onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `assistme onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `assistme onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `assistme onboard --auth-choice minimax-api`.
...
- Loaded from `~/.assistme/extensions`, `<workspace>/.assistme/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `assistme plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ASSISTME_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `ASSISTME_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `ASSISTME_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
Convenience flags: `--dev` (uses `~/.assistme-dev` + port `19001`), `--profile <name>` (uses `~/.assistme-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.assistme/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-assistme-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ASSISTME_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or ASSISTME_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__assistme__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__assistme__/a2ui/`
...
- Also serves A2UI at `/__assistme__/a2ui/`.
...
- Hostname defaults to `assistme`. Override with `ASSISTME_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.assistme/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `assistme dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.assistme/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ASSISTME_GATEWAY_TOKEN}" },
  },
}
```
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.assistme/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/assistme-vault-resolver",
...
- Legacy OAuth imports from `~/.assistme/credentials/oauth.json`.
...
- Default log file: `/tmp/assistme/assistme-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `assistme doctor --fix` can strip unknown keys).
