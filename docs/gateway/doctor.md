...
`assistme doctor` is the repair + migration tool for AssistMe. It fixes stale
...
```bash
assistme doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
assistme doctor --repair --force
```
...
- Extra workspace dir detection (`~/assistme`).
...
you to run `assistme doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.assistme/assistme.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.assistme/sessions/` to `~/.assistme/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.assistme/agent/` to `~/.assistme/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.assistme/credentials/*.json` (except `oauth.json`)
  - to `~/.assistme/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `assistme doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.assistme` folders exist across
  home directories or when `ASSISTME_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.assistme/assistme.json` is
...
offers to remove them and install the AssistMe service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named AssistMe gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `assistme doctor --generate-gateway-token` to force token
...
- `assistme doctor` prompts before rewriting supervisor config.
- `assistme doctor --yes` accepts the default repair prompts.
- `assistme doctor --repair` applies recommended fixes without prompts.
- `assistme doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `assistme gateway install --force`.
