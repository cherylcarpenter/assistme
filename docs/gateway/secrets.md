...
AssistMe supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, AssistMe performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). AssistMe validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/AssistMe QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/assistme"],
...
When reload-time activation fails after a healthy state, AssistMe enters degraded secrets state.
...
```bash
assistme secrets audit --check
assistme secrets configure
assistme secrets audit --check
```
...
- plaintext values at rest (`assistme.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `assistme.json`
...
- `assistme secrets configure --providers-only`
- `assistme secrets configure --skip-provider-setup`
...
```bash
assistme secrets apply --from /tmp/assistme-secrets-plan.json
assistme secrets apply --from /tmp/assistme-secrets-plan.json --dry-run
```
...
AssistMe intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, AssistMe runtime no longer depends on plaintext `auth.json`.
