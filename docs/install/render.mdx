...
Deploy AssistMe on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/assistme/assistme)
...
repository configures everything needed to run AssistMe:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ASSISTME_STATE_DIR
        value: /data/.assistme
      - key: ASSISTME_WORKSPACE_DIR
        value: /data/workspace
      - key: ASSISTME_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/assistme`.
...
If you use the original AssistMe repository, Render will not auto-deploy your AssistMe. To update it, run a manual Blueprint sync from the dashboard.
...
For AssistMe, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any AssistMe host.
