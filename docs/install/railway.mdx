...
Deploy AssistMe on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/assistme` — Control UI
...
- `ASSISTME_STATE_DIR=/data/.assistme` (recommended)
- `ASSISTME_WORKSPACE_DIR=/data/workspace` (recommended)
- `ASSISTME_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your AssistMe state + workspace so you can migrate to another host without losing config or memory.
