...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `assistme sandbox explain`.
...
Legacy `agent.*` configs are migrated by `assistme doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=assistme-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ASSISTME_STATE_DIR:-$HOME/.assistme}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
