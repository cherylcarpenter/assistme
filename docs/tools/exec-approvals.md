...
- **gateway host** → `assistme` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.assistme/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.assistme/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `assistme approvals allowlist ...`).
- `assistme security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `assistme doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.assistme/exec-approvals.json` directly.

CLI: `assistme approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
