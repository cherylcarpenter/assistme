---
summary: "Direct `assistme agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `assistme agent` (direct agent runs)

`assistme agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `assistme message --target`).
...
```bash
assistme agent --to +15555550123 --message "status update"
...
assistme agent --to +15555550123 --message "Summon reply" --deliver
