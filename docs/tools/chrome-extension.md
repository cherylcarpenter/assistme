---
summary: "Chrome extension: let AssistMe drive your existing Chrome tab"
read_when:
...
The AssistMe Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate assistme-managed Chrome profile.
...
AssistMe then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the AssistMe release (npm package) as static files. There is no separate “build” step.

After upgrading AssistMe:

- Re-run `assistme browser extension install` to refresh the installed files under your AssistMe state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

AssistMe ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `ASSISTME_GATEWAY_TOKEN`)
...
- Open the tab you want AssistMe to control.
...
- `ON`: attached; AssistMe can drive that tab.
...
Debugging: `assistme sandbox explain`
...
`assistme browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `assistme browser extension install` first to copy the extension to a stable location under your AssistMe state directory.
...
- **This is not isolated** like the dedicated assistme-managed profile.
