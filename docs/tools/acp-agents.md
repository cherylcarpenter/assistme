...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let AssistMe run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask AssistMe in plain language to "run this in Codex" or "start Claude Code in a thread", AssistMe should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want AssistMe-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | AssistMe native sub-agent runtime  |
...
- AssistMe binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, AssistMe returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, AssistMe uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, AssistMe may default to persistent behavior per runtime path
...
If no target resolves, AssistMe returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, AssistMe returns a clear unsupported-control error.
...
When AssistMe uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal AssistMe `agentId` path).
...
```bash
assistme plugins install @assistme/acpx
assistme config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
assistme plugins install ./extensions/acpx
```
...
By default, `@assistme/acpx` uses the plugin-local pinned binary:
...
- Relative paths resolve from AssistMe workspace directory.
...
- AssistMe startup remains non-blocking while the backend health check runs.
...
```bash
assistme config set plugins.entries.acpx.config.permissionMode approve-all
assistme config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** AssistMe currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>
