...
- With Anthropic API-key auth profiles, AssistMe seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, AssistMe injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
AssistMe exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.assistme/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$ASSISTME_STATE_DIR/logs/cache-trace.jsonl`
...
- `ASSISTME_CACHE_TRACE=1` enables cache tracing.
- `ASSISTME_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `ASSISTME_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `ASSISTME_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `ASSISTME_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
