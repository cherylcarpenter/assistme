...
AssistMe features that can generate provider usage or paid API calls.
...
- `assistme status --usage` and `assistme channels list` show provider **usage windows**
...
AssistMe can pick up credentials from:
...
- `assistme status --usage`
- `assistme models status --json`
...
`assistme models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
