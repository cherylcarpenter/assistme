...
AssistMe integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- AssistMe owns lifecycle when `channels.signal.autoStart=true`.
...
- AssistMe spawns `imsg rpc` as a child process (legacy iMessage integration).
