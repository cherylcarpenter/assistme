...
```bash
assistme plugins install @assistme/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
assistme plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "AssistMe" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `assistme pairing list nextcloud-talk`
  - `assistme pairing approve nextcloud-talk <CODE>`
