---
summary: "Synology Chat webhook setup and AssistMe config"
read_when:
  - Setting up Synology Chat with AssistMe
...
```bash
assistme plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your AssistMe gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in AssistMe.
...
- Pairing approvals work with:
  - `assistme pairing list synology-chat`
  - `assistme pairing approve synology-chat <CODE>`
...
```bash
assistme message send --channel synology-chat --target 123456 --text "Hello from AssistMe"
assistme message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
