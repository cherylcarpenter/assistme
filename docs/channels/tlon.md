...
Tlon is a decentralized messenger built on Urbit. AssistMe connects to your Urbit ship and can
...
```bash
assistme plugins install @assistme/tlon
```

Local checkout (when running from a git repo):

```bash
assistme plugins install ./extensions/tlon
```
...
By default, AssistMe blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `assistme message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AssistMe replies in-thread.
