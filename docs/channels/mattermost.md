---
summary: "Mattermost bot setup and AssistMe config"
read_when:
...
```bash
assistme plugins install @assistme/mattermost
```

Local checkout (when running from a git repo):

```bash
assistme plugins install ./extensions/mattermost
```
...
4. Configure AssistMe and start the gateway.
...
- Approve via:
  - `assistme pairing list mattermost`
  - `assistme pairing approve mattermost <CODE>`
...
Use these target formats with `assistme message send` or cron/webhooks:
