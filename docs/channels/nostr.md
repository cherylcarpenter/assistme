...
read_when:
  - You want AssistMe to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables AssistMe to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`assistme onboard`) and `assistme channels add` list optional channel plugins.
...
```bash
assistme plugins install @assistme/nostr
```

Use a local checkout (dev workflows):

```bash
assistme plugins install --link <path-to-assistme>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "assistme@example.com",
        "lud16": "assistme@example.com"
      }
    }
  }
}
