...
Matrix is an open, decentralized messaging protocol. AssistMe connects as a Matrix **user**
...
```bash
assistme plugins install @assistme/matrix
```

Local checkout (when running from a git repo):

```bash
assistme plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `assistme plugins install @assistme/matrix`
   - From a local checkout: `assistme plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AssistMe calls the same
     login endpoint, stores the access token in `~/.assistme/credentials/matrix/credentials.json`,
...
- On first connection, AssistMe requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  AssistMe logs a warning.
...
`~/.assistme/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `assistme pairing list matrix`
  - `assistme pairing approve matrix <CODE>`
...
- On startup, AssistMe resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
