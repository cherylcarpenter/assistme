...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects AssistMe to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
assistme plugins install @assistme/feishu
```

Local checkout (when running from a git repo):

```bash
assistme plugins install ./extensions/feishu
```
...
If you just installed AssistMe, run the wizard:
...
2. Configuring app credentials in AssistMe
...
1. You already ran `assistme channels add` for Feishu
2. The gateway is running (`assistme gateway status`)
...
```bash
assistme pairing approve feishu <CODE>
```
...
2. Run `assistme logs --follow` and look for `chat_id`
...
2. Run `assistme logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `assistme gateway status`  | Show gateway status           |
| `assistme gateway install` | Install/start gateway service |
| `assistme gateway stop`    | Stop gateway service          |
| `assistme gateway restart` | Restart gateway service       |
| `assistme logs --follow`   | Tail gateway logs             |
...
4. Check logs: `assistme logs --follow`
...
5. Ensure the gateway is running: `assistme gateway status`
6. Check logs: `assistme logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.assistme/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.assistme/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
