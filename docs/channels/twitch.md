...
read_when:
  - Setting up Twitch chat integration for AssistMe
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. AssistMe connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
assistme plugins install @assistme/twitch
```

Local checkout (when running from a git repo):

```bash
assistme plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `ASSISTME_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "assistme", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ASSISTME_TWITCH_ACCESS_TOKEN env var)
...
```bash
ASSISTME_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
