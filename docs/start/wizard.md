...
The onboarding wizard is the **recommended** way to set up AssistMe on macOS,
...
`assistme dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `assistme configure --section web`
...
2. **Workspace** — Location for agent files (default `~/.assistme/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `assistme doctor` first.
</Note>
...
Use `assistme agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.assistme/workspace-<agentId>`.
...
- CLI command reference: [`assistme onboard`](/cli/onboard)
