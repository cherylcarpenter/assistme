...
- `/usage cost` in chats: local cost summary aggregated from AssistMe session logs.
- CLI: `assistme status --usage` prints a full per-provider breakdown.
- CLI: `assistme channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
