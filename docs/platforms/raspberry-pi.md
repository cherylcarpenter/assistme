---
summary: "AssistMe on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up AssistMe on a Raspberry Pi
  - Running AssistMe on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# AssistMe on Raspberry Pi

## Goal

Run a persistent, always-on AssistMe Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
assistme config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/assistme-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
export NODE_COMPILE_CACHE=/var/tmp/assistme-compile-cache
mkdir -p /var/tmp/assistme-compile-cache
export ASSISTME_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `ASSISTME_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running AssistMe, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit assistme
```

```ini
[Service]
Environment=ASSISTME_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/assistme-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep AssistMe state/cache on SSD-backed storage to avoid SD-card
...
Most AssistMe features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled assistme

# Enable if not
sudo systemctl enable assistme

# Start on boot
sudo systemctl start assistme
```
...
```bash
# Check logs
journalctl -u assistme --no-pager -n 100

# Common fix: rebuild
cd ~/assistme  # if using hackable install
npm run build
sudo systemctl restart assistme
```
