...
- You can run the CLI (`assistme`) on the gateway machine (or via SSH).
...
- Set `gateway.bind: "tailnet"` in `~/.assistme/assistme.json` on the gateway host.
...
1. Set up a DNS-SD zone (example `assistme.internal.`) on the gateway host and publish `_assistme-gw._tcp` records.
...
1. Create `~/.assistme/workspace/canvas/index.html` on the gateway host.

2. Navigate the node to it (LAN):

```bash
assistme nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__assistme__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__assistme__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes.
The A2UI host lives at `http://<gateway-host>:18789/__assistme__/a2ui/`.
