...
- Same path: run the app from a fixed location (for AssistMe, `dist/AssistMe.app`).
...
```bash
sudo tccutil reset Accessibility ai.assistme.mac
sudo tccutil reset ScreenCapture ai.assistme.mac
...
Workaround: move files into the AssistMe workspace (`~/.assistme/workspace`) if you want to avoid per-folder grants.
