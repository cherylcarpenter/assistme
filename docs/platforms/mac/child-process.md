...
service via the external `assistme` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.assistme.gateway`
  (or `ai.assistme.<profile>` when using `--profile`/`ASSISTME_PROFILE`; legacy `com.assistme.*` is supported).
