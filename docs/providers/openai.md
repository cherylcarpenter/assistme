---
summary: "Use OpenAI via API keys or Codex subscription in AssistMe"
read_when:
  - You want to use OpenAI models in AssistMe
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like AssistMe.
...
```bash
assistme onboard --auth-choice openai-api-key
# or non-interactive
assistme onboard --openai-api-key "$OPENAI_API_KEY"
```
...
assistme onboard --auth-choice openai-codex

# Or run OAuth directly
assistme models auth login --provider openai-codex
```
...
AssistMe uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), AssistMe also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. AssistMe enables it by default for
...
`baseUrl` on `api.openai.com`), AssistMe now auto-enables OpenAI server-side
