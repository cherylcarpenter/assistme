---
summary: "Use Venice AI privacy-focused models in AssistMe"
read_when:
  - You want privacy-focused inference in AssistMe
...
```bash
assistme onboard --auth-choice venice-api-key
```
...
```bash
assistme agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, AssistMe shows all available Venice models. Pick based on your needs:
...
```bash
assistme models set venice/claude-opus-45
assistme models set venice/llama-3.3-70b
```

List all available models:

```bash
assistme models list | grep venice
```

## Configure via `assistme configure`

1. Run `assistme configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
AssistMe automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
assistme agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
assistme agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
assistme agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
assistme agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
assistme agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `assistme models list` to see currently available models. Some models may be temporarily offline.
