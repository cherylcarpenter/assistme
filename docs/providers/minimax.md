---
summary: "Use MiniMax M2.5 in AssistMe"
read_when:
  - You want MiniMax models in AssistMe
...
```bash
assistme plugins enable minimax-portal-auth  # skip if already loaded.
assistme gateway restart  # restart if gateway is already running
assistme onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/assistme/assistme/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `assistme.json`:
...
- Use `assistme models list` and `assistme models set minimax/MiniMax-M2.5` to switch.
...
- Running `assistme configure` and selecting **MiniMax M2.5**, or
