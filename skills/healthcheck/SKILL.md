---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for AssistMe deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, AssistMe cron scheduling for periodic checks, or version status checks on a machine running AssistMe (laptop, workstation, Pi, VPS).
---

# AssistMe Host Hardening

## Overview

Assess and harden the host running AssistMe, then align it to a user-defined risk tolerance without breaking access. Use AssistMe security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim AssistMe changes the host firewall, SSH, or OS updates; it does not.
...
5. AssistMe gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if AssistMe can access sensitive data.
...
### 2) Run AssistMe security audits (read-only)

As part of the default read-only checks, run `assistme security audit --deep`. Only offer alternatives if the user requests them:

1. `assistme security audit` (faster, non-probing)
2. `assistme security audit --json` (structured output)

Offer to apply AssistMe safe defaults (numbered):

1. `assistme security audit --fix`

Be explicit that `--fix` only tightens AssistMe defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check AssistMe version/update status (read-only)

As part of the default read-only checks, run `assistme update status`.
...
- Credential hygiene notes (location of AssistMe creds, prefer disk encryption)
...
- AssistMe security audit (re-run)
...
After AssistMe install or first hardening pass, run at least one baseline audit and version check:

- `assistme security audit`
- `assistme security audit --deep`
- `assistme update status`

Ongoing monitoring is recommended. Use the AssistMe cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `assistme cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `assistme update status`
...
Before creating, `assistme cron list` and match on exact `name`. If found, `assistme cron edit <id> ...`.
If not found, `assistme cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `assistme update status` (preferred for source checkouts and channels)
2. `npm view assistme version` (published npm version)

## AssistMe command accuracy

Use only supported commands and flags:

- `assistme security audit [--deep] [--fix] [--json]`
- `assistme status` / `assistme status --deep`
- `assistme health --json`
- `assistme update status`
- `assistme cron add|list|runs|run`

Do not invent CLI flags or imply AssistMe enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by AssistMe compaction:
