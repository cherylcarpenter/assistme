...
This document describes how the AssistMe ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`assistme acp` exposes an ACP agent over stdio and forwards prompts to a running
AssistMe Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `assistme acp` and speaks ACP messages over stdio.
...
`assistme acp` resolves the Gateway URL and auth from CLI flags or config:
