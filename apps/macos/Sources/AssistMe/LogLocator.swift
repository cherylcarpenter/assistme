...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ASSISTME_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/assistme")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("assistme-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("assistme-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/assistme/ (rolling or stdout), or nil if none exist.
