...
enum DebugActions {
    private static let verboseDefaultsKey = "assistme.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "assistme.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = AssistMePaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "AssistMe", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = AssistMePaths.configURL
        guard
