...
enum CommandResolver {
    private static let projectRootDefaultsKey = "assistme.gatewayProjectRootPath"
    private static let helperName = "assistme"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let assistmeEntry = root.appendingPathComponent("assistme.mjs").path
        if FileManager().isReadableFile(atPath: assistmeEntry) { return assistmeEntry }
        let binEntry = root.appendingPathComponent("bin/assistme.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/assistme")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let assistmePaths = self.assistmeManagedPaths(home: home)
        if !assistmePaths.isEmpty {
            extras.insert(contentsOf: assistmePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + assistmePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func assistmeManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".assistme"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func assistmeExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAssistMeExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("assistme.mjs").path,
            root.appendingPathComponent("bin/assistme.js").path,
        ]
    ...
    static func hasAnyAssistMeInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.assistmeExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func assistmeNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let assistmePath = self.projectAssistMeExecutable(projectRoot: root) {
            return [assistmePath, subcommand] + extraArgs
        }
        if let assistmePath = self.assistmeExecutable(searchPaths: searchPaths) {
            return [assistmePath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "assistme", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            assistme entrypoint missing (looked for dist/index.js or assistme.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real assistme CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/assistme"
        ...
        \(cliSection)
        \(projectSection)
        if command -v assistme >/dev/null 2>&1; then
          CLI="$(command -v assistme)"
          assistme \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/assistme.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/assistme.mjs"
            node "$PRJ/assistme.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/assistme.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/assistme.js"
            node "$PRJ/bin/assistme.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent assistme"
          pnpm --silent assistme \(quotedArgs);
        else
          echo "assistme CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AssistMeConfigFile.loadDict()
