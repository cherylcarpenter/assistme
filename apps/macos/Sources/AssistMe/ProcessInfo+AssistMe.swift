...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.assistme.mac`) even if the shipped
    ...
    {
        if environment["ASSISTME_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "assistme.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "assistme.nixMode") { return true }
