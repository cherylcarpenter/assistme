...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: AssistMeProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.assistme", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AssistMeKit.AnyCodable]()) {
                $0[$1.key] = AssistMeKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AssistMeProtocol.AnyCodable?) -> [String: AssistMeProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AssistMeProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AssistMeKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AssistMeProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("assistme.control.heartbeat")
    static let controlAgentEvent = Notification.Name("assistme.control.agent")
}
