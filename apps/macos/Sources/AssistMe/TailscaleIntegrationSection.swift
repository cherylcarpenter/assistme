...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.assistme/assistme.json. Prefer ASSISTME_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.assistme/assistme.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.assistme/assistme.json. Restart the gateway to apply."
        }
