...
enum AssistMeLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = AssistMeOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AssistMeFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AssistMeLogging.bootstrapIfNeeded()
        let label = AssistMeLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AssistMeOSLogHandler: AppLogLevelBackedHandler {
...
struct AssistMeFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AssistMeLogging.parseLabel(self.label)
