...
private let webChatSwiftLogger = Logger(subsystem: "ai.assistme", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: AssistMeChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AssistMeChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(AssistMeChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AssistMeChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AssistMeGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    AssistMeAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AssistMeChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AssistMeChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AssistMeChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: AssistMeChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AssistMeChatView>) -> NSViewController
    {
