...
private let deepLinkLogger = Logger(subsystem: "ai.assistme", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "AssistMe is paused", message: "Unpause AssistMe to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run AssistMe agent?", message: body) else { return }
        }
