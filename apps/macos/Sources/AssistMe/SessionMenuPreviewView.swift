...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.assistme", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AssistMeSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AssistMeSessionPreviewEntry,
    ...
    private static func cache(payload: AssistMeSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: AssistMeChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AssistMeKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AssistMeKit.AnyCodable]) -> [AssistMeChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AssistMeChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AssistMeChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AssistMeChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AssistMeChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AssistMeChatMessage) -> String? {
