...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["assistme"]
        let text = "hey assistme"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey assistme do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["assistme"], minPostTriggerGap: 0.3)
