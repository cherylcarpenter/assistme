...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: AssistMeProtocol.AnyCodable(["ok": AssistMeProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: AssistMeProtocol.AnyCodable(["ok": AssistMeProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = AssistMeProtocol.AnyCodable([
            "runId": AssistMeProtocol.AnyCodable("run-1"),
            "sessionKey": AssistMeProtocol.AnyCodable("main"),
            "state": AssistMeProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: AssistMeProtocol.AnyCodable(["a": AssistMeProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
