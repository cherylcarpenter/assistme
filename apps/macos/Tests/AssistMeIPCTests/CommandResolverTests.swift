...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersAssistMeBinary() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()
        CommandResolver.setProjectRoot(tmp.path)

        let assistmePath = tmp.appendingPathComponent("node_modules/.bin/assistme")
        try makeExecutableForTests(at: assistmePath)

        let cmd = CommandResolver.assistmeCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([assistmePath.path, "gateway"]))
    }
    ...
    @Test func prefersAssistMeBinaryOverPnpm() throws {
        let defaults = self.makeLocalDefaults()
        ...
        let assistmePath = binDir.appendingPathComponent("assistme")
        ...
        #expect(cmd.prefix(2).elementsEqual([assistmePath.path, "rpc"]))
    }

    @Test func usesAssistMeBinaryWithoutNodeRuntime() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func fallsBackToPnpm() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "assistme", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "assistme", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() {
        ...
        defaults.set("assistme@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/assistme", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "assistme@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/assistme'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("assistme"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() throws {
        ...
        #expect(cmd.first == assistmePath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
