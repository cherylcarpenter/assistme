...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: AssistMeProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: AssistMeProtocol.AnyCodable] = [
            "presence": .init([AssistMeProtocol.AnyCodable(entry)]),
        ]
        let payload = AssistMeProtocol.AnyCodable(payloadMap)
