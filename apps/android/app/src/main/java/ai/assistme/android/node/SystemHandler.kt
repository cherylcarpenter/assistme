...
private const val NOTIFICATION_CHANNEL_BASE_ID = "assistme.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "AssistMe Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "AssistMe Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "AssistMe Active")
      }
