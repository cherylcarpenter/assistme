...
import Foundation
import Testing
@testable import AssistMeChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> AssistMeChatHistoryPayload
{
    AssistMeChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> AssistMeChatSessionEntry {
...
private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [AssistMeChatHistoryPayload],
    sessionsResponses: [AssistMeChatSessionsListResponse] = []) async -> (TestChatTransport, AssistMeChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { AssistMeChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}

private func loadAndWaitBootstrap(
    vm: AssistMeChatViewModel,
...
private func sendUserMessage(_ vm: AssistMeChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, AssistMeChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AssistMeChatHistoryPayload]
    private let sessionsResponses: [AssistMeChatSessionsListResponse]

    private let stream: AsyncStream<AssistMeChatTransportEvent>
    private let continuation: AsyncStream<AssistMeChatTransportEvent>.Continuation

    init(
        historyResponses: [AssistMeChatHistoryPayload],
        sessionsResponses: [AssistMeChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<AssistMeChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> AssistMeChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AssistMeChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [AssistMeChatAttachmentPayload]) async throws -> AssistMeChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return AssistMeChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AssistMeChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AssistMeChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: AssistMeChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = AssistMeChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        let history = AssistMeChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [
                AnyCodable([
...
"""]],
        ...
        let vm = await MainActor.run { AssistMeChatViewModel(sessionKey: "main", transport: transport) }
