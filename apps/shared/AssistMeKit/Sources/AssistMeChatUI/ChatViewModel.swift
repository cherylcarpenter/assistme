...
private let chatUILogger = Logger(subsystem: "ai.assistme", category: "AssistMeChatUI")

@MainActor
@Observable
public final class AssistMeChatViewModel {
    public private(set) var messages: [AssistMeChatMessage] = []
    ...
    public var attachments: [AssistMePendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AssistMeChatPendingToolCall] = []
    public private(set) var sessions: [AssistMeChatSessionEntry] = []
    private let transport: any AssistMeChatTransport
    ...
    private var pendingToolCallsById: [String: AssistMeChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any AssistMeChatTransport) {
    ...
    public var sessionChoices: [AssistMeChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [AssistMeChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AssistMePendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AssistMeChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AssistMeChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: AssistMeChatMessage) -> AssistMeChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> AssistMeChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return AssistMeChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: AssistMeChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [AssistMeChatMessage],
        incoming: [AssistMeChatMessage]) -> [AssistMeChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [AssistMeChatMessage]) -> [AssistMeChatMessage] {
        var result: [AssistMeChatMessage] = []
    ...
    private static func dedupeKey(for message: AssistMeChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AssistMeChatMessageContent] = [
            AssistMeChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> AssistMeChatAttachmentPayload in
            AssistMeChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AssistMeChatSessionEntry {
        AssistMeChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: AssistMeChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: AssistMeChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AssistMeAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AssistMeChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AssistMePlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
