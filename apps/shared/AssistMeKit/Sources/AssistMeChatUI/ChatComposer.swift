...
@MainActor
struct AssistMeChatComposer: View {
    @Bindable var viewModel: AssistMeChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AssistMeChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AssistMeChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \AssistMePendingAttachment.id)
                { (att: AssistMePendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            AssistMePlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(AssistMeChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AssistMeChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(AssistMeChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(AssistMeChatTheme.subtleCard)
        .clipShape(Capsule())
    }
