...
struct ChatMessageBubble: View {
    let message: AssistMeChatMessage
    let style: AssistMeChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? AssistMeChatTheme.userText : AssistMeChatTheme.assistantText
    ...
    private var inlineAttachments: [AssistMeChatMessageContent] {
    ...
    private var toolCalls: [AssistMeChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [AssistMeChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? AssistMeChatTheme.userBubble
        }
        if self.style == .onboarding {
            return AssistMeChatTheme.onboardingAssistantBubble
        }
        return AssistMeChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AssistMeChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: AssistMeChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AssistMeChatTheme.userText : AssistMeChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: AssistMeChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AssistMeChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(AssistMeChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [AssistMeChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: AssistMeChatTheme.assistantText)
            }
        }
    }
}
