...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.assistme.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to AssistMe", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to AssistMe gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to AssistMe.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "AssistMeShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "AssistMe is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "AssistMe Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("assistme-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
