...
@MainActor
final class AssistMeAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.assistme.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.assistme.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.assistme.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "assistme.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "assistme.watch.promptId"
    static let sessionKeyKey = "assistme.watch.sessionKey"
    static let actionPrimaryIDKey = "assistme.watch.action.primary.id"
    static let actionPrimaryLabelKey = "assistme.watch.action.primary.label"
    static let actionSecondaryIDKey = "assistme.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "assistme.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "assistme.watch.action.primary"
    static let actionSecondaryIdentifier = "assistme.watch.action.secondary"
    static let actionIdentifierPrefix = "assistme.watch.action."
    static let actionIDKeyPrefix = "assistme.watch.action.id."
    static let actionLabelKeyPrefix = "assistme.watch.action.label."
    static let categoryPrefix = "assistme.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: AssistMeWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> AssistMeWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AssistMeWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "AssistMe" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [AssistMeWatchAction]) -> [UNNotificationAction] {
...
@main
struct AssistMeApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(AssistMeAppDelegate.self) private var appDelegate
...
extension AssistMeApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("AssistMe: installing uncaught exception handler")
