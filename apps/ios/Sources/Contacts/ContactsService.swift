...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AssistMeContactsSearchParams) async throws -> AssistMeContactsSearchPayload {
        ...
        return AssistMeContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AssistMeContactsAddParams) async throws -> AssistMeContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AssistMeContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AssistMeContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AssistMeContactPayload {
        AssistMeContactPayload(
