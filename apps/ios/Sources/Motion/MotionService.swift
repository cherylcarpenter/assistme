...
final class MotionService: MotionServicing {
    func activities(params: AssistMeMotionActivityParams) async throws -> AssistMeMotionActivityPayload {
        ...
        let mapped: [AssistMeMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return AssistMeMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AssistMePedometerParams) async throws -> AssistMePedometerPayload {
        ...
        let payload: AssistMePedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AssistMePedometerPayload(
