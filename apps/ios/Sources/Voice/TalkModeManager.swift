...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ASSISTME_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<AssistMeTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.assistme", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = AssistMeTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> AssistMeTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return AssistMeTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> AssistMeTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> AssistMeTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return AssistMeTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> AssistMeTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = AssistMeTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: AssistMeTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: AssistMeAgentEventPayload.self
            ) else {
                continue
            }
