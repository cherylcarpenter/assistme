...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: AssistMeWatchNotifyParams) -> AssistMeWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [AssistMeWatchAction]?,
        kind: String?,
        promptId: String?) -> [AssistMeWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> AssistMeWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AssistMeWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                AssistMeWatchAction(id: "approve", label: "Approve"),
                AssistMeWatchAction(id: "decline", label: "Decline", style: "destructive"),
                AssistMeWatchAction(id: "open_phone", label: "Open iPhone"),
                AssistMeWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            AssistMeWatchAction(id: "done", label: "Done"),
            AssistMeWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            AssistMeWatchAction(id: "open_phone", label: "Open iPhone"),
            AssistMeWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: AssistMeWatchRisk?,
        priority: AssistMeNotificationPriority?) -> AssistMeWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: AssistMeNotificationPriority?,
        risk: AssistMeWatchRisk?) -> AssistMeNotificationPriority?
    {
