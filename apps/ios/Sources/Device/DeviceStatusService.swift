...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> AssistMeDeviceStatusPayload {
        ...
        return AssistMeDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> AssistMeDeviceInfoPayload {
        ...
        return AssistMeDeviceInfoPayload(
    ...
    private func batteryStatus() -> AssistMeBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: AssistMeBatteryState = switch device.batteryState {
        ...
        return AssistMeBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> AssistMeThermalStatusPayload {
        let state: AssistMeThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return AssistMeThermalStatusPayload(state: state)
    }

    private func storageStatus() -> AssistMeStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return AssistMeStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
