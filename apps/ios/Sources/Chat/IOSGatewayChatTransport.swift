...
struct IOSGatewayChatTransport: AssistMeChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.assistme", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> AssistMeChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AssistMeChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AssistMeChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AssistMeChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AssistMeChatAttachmentPayload]) async throws -> AssistMeChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AssistMeChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(AssistMeChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(AssistMeGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AssistMeGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AssistMeChatEventPayload.self)
                        {
