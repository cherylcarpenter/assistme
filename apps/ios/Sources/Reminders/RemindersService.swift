...
final class RemindersService: RemindersServicing {
    func list(params: AssistMeRemindersListParams) async throws -> AssistMeRemindersListPayload {
        ...
        let payload: [AssistMeReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return AssistMeReminderPayload(
        ...
        return AssistMeRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AssistMeRemindersAddParams) async throws -> AssistMeRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AssistMeReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AssistMeRemindersAddPayload(reminder: payload)
    }
