...
#
# Suggested cron: */30 * * * * /home/admin/assistme/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AssistMe notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., assistme-alerts)
...
STATE_FILE="$HOME/.assistme/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AssistMe if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use assistme
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AssistMe to $NOTIFY_PHONE..."
            assistme send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AssistMe is down. Run: ssh l36 '~/assistme/scripts/mobile-reauth.sh'" "urgent"
    exit 1
