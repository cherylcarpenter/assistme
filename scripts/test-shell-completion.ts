 ...
 * `assistme update`. Use it to verify the completion installation flow
...
\`assistme update\`, \`assistme doctor\`, and \`assistme onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# AssistMe Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`assistme completion --install\` later to enable.`));
    return;
  }
