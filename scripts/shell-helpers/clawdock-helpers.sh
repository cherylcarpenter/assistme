#!/usr/bin/env bash
# ClawDock - Docker helpers for AssistMe
# Inspired by Simon Willison's "Running AssistMe in Docker"
# https://til.simonwillison.net/llms/assistme-docker
#
# Installation:
...
# Common paths to check for AssistMe
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/assistme"
  "${HOME}/workspace/assistme"
  "${HOME}/projects/assistme"
  "${HOME}/dev/assistme"
  "${HOME}/code/assistme"
  "${HOME}/src/assistme"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found AssistMe at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/assistme"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ AssistMe not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/assistme/assistme.git ~/assistme"
    echo "  cd ~/assistme && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ASSISTME_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d assistme-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart assistme-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f assistme-gateway
}
...
clawdock-workspace() {
  cd ~/.assistme/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec assistme-gateway \
    bash -c 'echo "alias assistme=\"./assistme.mjs\"" > /tmp/.bashrc_assistme && bash --rcfile /tmp/.bashrc_assistme'
}
...
clawdock-cli() {
  _clawdock_compose run --rm assistme-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build assistme-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ASSISTME_GATEWAY_TOKEN=$token" assistme-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" assistme-gateway \
    bash -c './assistme.mjs config set gateway.remote.token "$TOKEN" && ./assistme.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec assistme-gateway \
    bash -c "./assistme.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart assistme-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm assistme-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec assistme-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'assistme config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for AssistMe${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (assistme alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to assistme project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.assistme)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'assistme channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'assistme status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.assistme.ai${_CLR_RESET}"
  echo ""
}
