...
function runTests() {
  const isolatedLock =
    process.env.ASSISTME_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `assistme-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      ASSISTME_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.ASSISTME_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
