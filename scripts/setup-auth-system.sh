#!/bin/bash
# Setup AssistMe Auth Management System
...
echo "=== AssistMe Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'assistme-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  AssistMe message: Send warning via AssistMe itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/assistme-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/assistme-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/assistme-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now assistme-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status assistme-auth-monitor.timer"
echo ""
echo "Quick commands:"
