...
IMAGE_NAME="assistme-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="assistme-net-e2e-$$"
GW_NAME="assistme-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "ASSISTME_GATEWAY_TOKEN=$TOKEN" \
  -e "ASSISTME_SKIP_CHANNELS=1" \
  -e "ASSISTME_SKIP_GMAIL_WATCHER=1" \
  -e "ASSISTME_SKIP_CRON=1" \
  -e "ASSISTME_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
