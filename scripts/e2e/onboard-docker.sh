...
IMAGE_NAME="assistme-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    ASSISTME_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    ASSISTME_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export ASSISTME_ENTRY

  # Provide a minimal trash shim to avoid noisy "missing trash" logs in containers.
  export PATH="/tmp/assistme-bin:$PATH"
  mkdir -p /tmp/assistme-bin
  cat > /tmp/assistme-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/assistme-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$ASSISTME_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_assistme_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/assistme-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/assistme-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$ASSISTME_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/assistme-e2e-$1.XXXXXX"
  }

  set_isolated_assistme_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export ASSISTME_HOME="$home_dir"
    export ASSISTME_STATE_DIR="$home_dir/.assistme"
    export ASSISTME_CONFIG_PATH="$ASSISTME_STATE_DIR/assistme.json"
    mkdir -p "$ASSISTME_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$ASSISTME_STATE_DIR/workspace"
    config_path="$ASSISTME_CONFIG_PATH"
    sessions_dir="$ASSISTME_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_assistme_env "$home_dir"
	    ...
	    node "$ASSISTME_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$ASSISTME_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$ASSISTME_ENTRY\" configure --section skills" send_skills_flow
