# AssistMe Installer for Windows (PowerShell)
# Usage: iwr -useb https://assistme.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://assistme.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\assistme",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 AssistMe Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one AssistMe.$NC" -Level info
    Write-Host ""
}
...
function Install-AssistMeNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing AssistMe (assistme@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g assistme@$Version --no-fund --no-audit 2>&1
        Write-Host "AssistMe installed" -Level success
        return $true
    } catch {
...
function Install-AssistMeGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing AssistMe from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/assistme/assistme.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\assistme\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\assistme.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install AssistMe from git to $GitDir" -Level info
        } else {
            Install-AssistMeGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install AssistMe via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-AssistMeNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'assistme onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 AssistMe installed successfully!" -Level success
}
