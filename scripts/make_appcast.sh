...
ZIP=${1:?"Usage: $0 AssistMe-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/assistme/assistme/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^AssistMe-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/assistme/assistme/releases/download/v${VERSION}/"}
