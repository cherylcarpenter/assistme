...
type DriverMode = "token" | "webhook" | "assistme";
...
function resolveStateDir(): string {
  const override = process.env.ASSISTME_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.ASSISTME_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".assistme");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver assistme] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) AssistMe spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|assistme> Driver transport mode (default: token)\n" +
    ...
    "  --assistme-bin <path>        AssistMe CLI binary for driver=assistme (default: assistme)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  ASSISTME_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  ASSISTME_DISCORD_SMOKE_DRIVER\n" +
    "  ASSISTME_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  ASSISTME_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  ASSISTME_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  ASSISTME_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  ASSISTME_DISCORD_SMOKE_AGENT\n" +
    "  ASSISTME_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  ASSISTME_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  ASSISTME_DISCORD_SMOKE_POLL_MS\n" +
    "  ASSISTME_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  ASSISTME_DISCORD_SMOKE_ASSISTME_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.ASSISTME_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.ASSISTME_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.ASSISTME_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.ASSISTME_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.ASSISTME_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.ASSISTME_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.ASSISTME_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.ASSISTME_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.ASSISTME_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.ASSISTME_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.ASSISTME_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.ASSISTME_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const assistmeBin =
    resolveArg("--assistme-bin") || process.env.ASSISTME_DISCORD_SMOKE_ASSISTME_BIN || "assistme";
...
async function assistmeCliJson<T>(params: { assistmeBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.assistmeBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`assistme ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await assistmeCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    assistmeBin: params.assistmeBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "assistme") {
    return await readMessagesWithOpenclaw({
      assistmeBin: params.args.assistmeBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `assistme-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await assistmeCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("assistme message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from AssistMe.`,
        diagnostics: {
          bindingCandidates: [
            {
