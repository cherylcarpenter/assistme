...
INSTALL_URL="${ASSISTME_INSTALL_URL:-https://assistme.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${ASSISTME_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ASSISTME_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="assistme"
PACKAGE_NAME="${ASSISTME_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ASSISTME_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ASSISTME_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${ASSISTME_INSTALL_LATEST_OUT:-}"
fi
