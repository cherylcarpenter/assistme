...
BASE_IMAGE="${BASE_IMAGE:-assistme-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-assistme-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=assistme.sandbox=1)
NOTE
