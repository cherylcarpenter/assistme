...
const shouldBuild = (deps) => {
  if (deps.env.ASSISTME_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.ASSISTME_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[assistme] ${message}\n`);
};

const runAssistMe = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["assistme.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
