...
# Build and bundle AssistMe into a minimal .app we can open.
# Outputs to dist/AssistMe.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/AssistMe.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="AssistMe"
BUNDLE_ID="${BUNDLE_ID:-ai.assistme.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/assistme/assistme/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/AssistMe/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :AssistMeBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :AssistMeGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/AssistMe"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/AssistMe"
fi
chmod +x "$APP_ROOT/Contents/MacOS/AssistMe"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/AssistMe" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/AssistMe/Resources/AssistMe.icns" "$APP_ROOT/Contents/Resources/AssistMe.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/AssistMe/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying AssistMeKit resources"
ASSISTMEKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/AssistMeKit_AssistMeKit.bundle"
if [ -d "$ASSISTMEKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/AssistMeKit_AssistMeKit.bundle"
  cp -R "$ASSISTMEKIT_BUNDLE" "$APP_ROOT/Contents/Resources/AssistMeKit_AssistMeKit.bundle"
else
  echo "WARN: AssistMeKit resource bundle not found at $ASSISTMEKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running AssistMe"
killall -q AssistMe 2>/dev/null || true
