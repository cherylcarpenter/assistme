...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/assistme/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AssistMe Re-Auth" -c "After getting key, run: ssh $SERVER '~/assistme/scripts/mobile-reauth.sh'" --id assistme-auth
        ;;
    *)
