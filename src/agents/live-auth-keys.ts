...
const GOOGLE_LIVE_SINGLE_KEY = "ASSISTME_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ASSISTME_LIVE_ANTHROPIC_KEY",
    listVar: "ASSISTME_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ASSISTME_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ASSISTME_LIVE_${base}_KEY`;
