...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureAssistMeModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): AssistMeConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): AssistMeConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): AssistMeConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-workspace-"));
    const now = Date.now();
