...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: AssistMeConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/assistme" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: AssistMeConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: AssistMeConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: AssistMeConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: AssistMeConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: AssistMeConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: AssistMeConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: AssistMeConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/assistme");
  });

  it("uses ASSISTME_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "assistme-home");
    vi.stubEnv("ASSISTME_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as AssistMeConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".assistme", "workspace"));
  });

  it("uses ASSISTME_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "assistme-home");
    vi.stubEnv("ASSISTME_HOME", home);
    // Clear state dir so it falls back to ASSISTME_HOME
    vi.stubEnv("ASSISTME_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as AssistMeConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".assistme", "agents", "main", "agent"));
  });
});
