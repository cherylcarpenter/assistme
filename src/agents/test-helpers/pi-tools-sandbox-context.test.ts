...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("assistme-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("assistme-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("assistme-sbx-");
