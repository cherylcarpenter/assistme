...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/assistme/assistme/issues/31298
    previousFastTestEnv = process.env.ASSISTME_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.ASSISTME_TEST_FAST;
      return;
    }
    process.env.ASSISTME_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // ASSISTME_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("AssistMe runtime context (internal):");
