...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("assistme-ws-", async (workspaceDir) => {
      await withTempDir("assistme-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createAssistMeCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("assistme-ws-", async (workspaceDir) => {
      await withTempDir("assistme-override-", async (overrideDir) => {
        const tools = createAssistMeCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("assistme-ws-", async (workspaceDir) => {
      const cfg: AssistMeConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createAssistMeCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-assistme.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("assistme-sandbox-", async (sandboxDir) => {
      await withTempDir("assistme-workspace-", async (workspaceDir) => {
        ...
        const tools = createAssistMeCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
