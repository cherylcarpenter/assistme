...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "assistme-pi-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.ASSISTME_AUTH_STORE_READONLY;
      ...
      try {
        ...
        if (previous === undefined) {
          ...
          process.env.ASSISTME_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });
