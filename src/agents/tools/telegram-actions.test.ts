...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): AssistMeConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as AssistMeConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): AssistMeConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as AssistMeConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: AssistMeConfig, accountId = "media") {
    await handleTelegramAction(
