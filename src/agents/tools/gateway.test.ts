...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    assistme: process.env.ASSISTME_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.assistme === undefined) {
      delete process.env.ASSISTME_GATEWAY_TOKEN;
    } else {
      process.env.ASSISTME_GATEWAY_TOKEN = envSnapshot.assistme;
    }
  ...
  it("uses ASSISTME_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ASSISTME_GATEWAY_TOKEN = "local-env-token";
