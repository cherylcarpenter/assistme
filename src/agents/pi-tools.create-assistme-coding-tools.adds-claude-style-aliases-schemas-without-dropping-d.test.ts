...
describe("createAssistMeCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-read-"));
    try {
      ...
      const contents = "Hello from assistme read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createAssistMeCodingTools({ sandbox });
