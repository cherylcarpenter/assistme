...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const assistMeTmpDir = path.resolve(resolvePreferredAssistMeTmpDir());
  if (!isPathInside(assistMeTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: assistMeTmpDir });
  return resolved;
}
