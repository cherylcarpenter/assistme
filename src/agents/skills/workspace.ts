...
function resolveSkillsLimits(config?: AssistMeConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "assistme-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "assistme-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveAssistMeMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: AssistMeConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`assistme skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
