    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "assistme.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "assistme.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "assistme.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("assistme-");
  const acpxRoot = await tempDirs.make("assistme-acpx-plugin-");
  const helperRoot = await tempDirs.make("assistme-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("assistme-");
  const pluginRoot = await tempDirs.make("assistme-plugin-");
  const outsideDir = await tempDirs.make("assistme-outside-");
