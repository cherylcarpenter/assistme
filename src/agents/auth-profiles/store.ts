 ...
 * A common mistake — caused by the similarity with the `assistme.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (assistme.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.ASSISTME_AUTH_STORE_READONLY === "1";
