...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ASSISTME_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ASSISTME_HOME: "/srv/assistme-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/assistme-home"), ".assistme", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".assistme", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("assistme-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-workspace-hardlink-"));
    try {
