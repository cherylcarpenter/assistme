...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createAssistMeTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-test-"));

    try {
      await withEnvAsync(
        { ASSISTME_STATE_DIR: stateDir, ASSISTME_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: assistme --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/assistme" } }\n}\n';
