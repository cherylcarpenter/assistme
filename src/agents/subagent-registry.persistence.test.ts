...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["ASSISTME_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-subagent-"));
  ...
  it("uses isolated temp state when ASSISTME_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "assistme-test-state"));
  });
});
