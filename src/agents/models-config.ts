...
type ModelsConfig = NonNullable<AssistMeConfig["models"]>;
...
export async function ensureAssistMeModelsJson(
  config?: AssistMeConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveAssistMeAgentDir();
