...
type CoreToolDefinition = {
  ...
  includeInAssistMeGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const assistmeTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInAssistMeGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:assistme": assistmeTools,
