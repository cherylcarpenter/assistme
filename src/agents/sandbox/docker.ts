...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "assistme.sandbox=1");
  args.push("--label", `assistme.sessionKey=${params.scopeKey}`);
  args.push("--label", `assistme.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `assistme.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "assistme.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`assistme sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`assistme sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("assistme sandbox recreate --all");
}
