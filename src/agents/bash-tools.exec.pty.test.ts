...
test("exec sets ASSISTME_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-assistme-shell", {
    command: "node -e \"process.stdout.write(process.env.ASSISTME_SHELL || '')\"",
    pty: true,
  });
