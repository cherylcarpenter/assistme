...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.ASSISTME_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".assistme", `workspace-${profile}`);
  }
  return path.join(home, ".assistme", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".assistme";
