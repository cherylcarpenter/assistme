...
import { createAssistMeCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: AssistMeConfig) {
    return createAssistMeCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<AssistMeConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<AssistMeConfig["agents"]>["list"]>[number]["tools"];
  }): AssistMeConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createAssistMeCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createAssistMeCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createAssistMeCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createAssistMeCodingTools({
      config: cfg,
    ...
    const bobTools = createAssistMeCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createAssistMeCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createAssistMeCodingTools({
      config: cfg,
