...
export function isCommandEnabled(cfg: AssistMeConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AssistMeConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as AssistMeConfig),
...
export function getCommandDetection(_cfg?: AssistMeConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AssistMeConfig) {
