...
import { scheduleGatewaySigusr1Restart, triggerAssistMeRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting AssistMe in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAssistMeRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting AssistMe via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
