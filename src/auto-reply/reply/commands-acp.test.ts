...
function createDiscordParams(commandBody: string, cfg: AssistMeConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: AssistMeConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

async function runDiscordAcpCommand(commandBody: string, cfg: AssistMeConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: AssistMeConfig = baseCfg) {
