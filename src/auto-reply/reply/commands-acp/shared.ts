...
export function resolveConfiguredAcpBackendId(cfg: AssistMeConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: AssistMeConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `assistme plugins install ${localPath}`;
    }
    return "assistme plugins install @assistme/acpx";
  }
