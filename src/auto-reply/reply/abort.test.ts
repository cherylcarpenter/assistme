...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop assistme!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@assistme_bot", { botUsername: "assistme_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@assistme_bot", { botUsername: "assistme_bot" })).toBe(true);
