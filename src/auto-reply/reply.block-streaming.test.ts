...
function createReplyConfig(home: string, streamMode?: "block"): AssistMeConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("assistme-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("ASSISTME_TEST_FAST", "1");
