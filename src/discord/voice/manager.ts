...
function resolveVoiceTtsConfig(params: { cfg: AssistMeConfig; override?: TtsConfig }): {
  cfg: AssistMeConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredAssistMeTmpDir(), "discord-voice-"));
