...
let originalAssistMeLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.ASSISTME_TEST_FILE_LOG;
  originalAssistMeLogLevel = process.env.ASSISTME_LOG_LEVEL;
  delete process.env.ASSISTME_TEST_FILE_LOG;
  delete process.env.ASSISTME_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.ASSISTME_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalAssistMeLogLevel === undefined) {
    delete process.env.ASSISTME_LOG_LEVEL;
  } else {
    process.env.ASSISTME_LOG_LEVEL = originalAssistMeLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
