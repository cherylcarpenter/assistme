    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "assistme node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["assistme node status", "Check node host service status."],
          ["assistme node install", "Install the node host service."],
          ["assistme node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.assistme.ai/cli/node")}\n`,
    );
