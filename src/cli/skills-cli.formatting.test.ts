...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "assistme-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "assistme-bundled-skills-test-"));
    process.env.ASSISTME_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
