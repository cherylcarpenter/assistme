...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update AssistMe and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["assistme update", "Update a source checkout (git)"],
        ["assistme update --channel beta", "Switch to beta channel (git + npm)"],
        ["assistme update --channel dev", "Switch to dev channel (git + npm)"],
        ["assistme update --tag beta", "One-off update to a dist-tag or version"],
        ["assistme update --dry-run", "Preview actions without changing anything"],
        ["assistme update --no-restart", "Update without restarting the service"],
        ["assistme update --json", "Output result as JSON"],
        ["assistme update --yes", "Non-interactive (accept downgrade prompts)"],
        ["assistme update wizard", "Interactive update wizard"],
        ["assistme --update", "Shorthand for assistme update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run assistme update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.assistme.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["assistme update status", "Show channel + version status."],
          ["assistme update status --json", "JSON output."],
          ["assistme update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.assistme.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
