...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@assistme/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@assistme/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@assistme/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@assistme/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @assistme/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@assistme/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@assistme/plugin-alpha@1.2.3",
        installPath: "/tmp/assistme/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@assistme/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @assistme/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@assistme/plugin-alpha@latest",
      true,
      "/tmp/assistme/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@assistme/plugin-alpha@latest",
      installPath: "/tmp/assistme/extensions/alpha",
      version: "1.2.3",
