...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.assistme.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + AssistMe hooks")
    ...
    .option("--hook-url <url>", "AssistMe hook URL")
    .option("--hook-token <token>", "AssistMe hook token")
