...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    ASSISTME_STATE_DIR: "/tmp/assistme-daemon",
    ASSISTME_CONFIG_PATH: "/tmp/assistme-daemon/assistme.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.ASSISTME_STATE_DIR ?? "/tmp/assistme-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.ASSISTME_CONFIG_PATH ?? `${stateDir}/assistme.json`;
});

vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/assistme-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ASSISTME_STATE_DIR = "/tmp/assistme-cli";
    process.env.ASSISTME_CONFIG_PATH = "/tmp/assistme-cli/assistme.json";
