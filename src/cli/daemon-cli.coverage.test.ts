...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      ASSISTME_GATEWAY_PORT: String(params.port),
      ...(params.token ? { ASSISTME_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.ASSISTME_STATE_DIR = "/tmp/assistme-cli-state";
    process.env.ASSISTME_CONFIG_PATH = "/tmp/assistme-cli-state/assistme.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        ASSISTME_STATE_DIR: "/tmp/assistme-daemon-state",
        ASSISTME_CONFIG_PATH: "/tmp/assistme-daemon-state/assistme.json",
        ASSISTME_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.assistme.gateway.plist",
    });
