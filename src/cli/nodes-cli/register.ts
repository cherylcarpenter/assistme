    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["assistme nodes status", "List known nodes with live status."],
          ["assistme nodes pairing pending", "Show pending node pairing requests."],
          ['assistme nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["assistme nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.assistme.ai/cli/nodes")}\n`,
    );
