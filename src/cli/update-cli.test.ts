...
import type { AssistMeConfig, ConfigFileSnapshot } from "../config/types.assistme.js";
...
vi.mock("../infra/assistme-root.js", () => ({
  resolveAssistMePackageRoot: vi.fn(),
}));
...
const { resolveAssistMePackageRoot } = await import("../infra/assistme-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/assistme-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as AssistMeConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/assistme-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveAssistMePackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("assistme-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveAssistMePackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("assistme");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/assistme-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "assistme-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("AssistMe update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as AssistMeConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveAssistMePackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("assistme-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ASSISTME_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.ASSISTME_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("assistme-update-wizard");
    await withEnvAsync({ ASSISTME_GIT_DIR: tempDir }, async () => {
      setTty(true);
