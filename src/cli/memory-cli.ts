    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["assistme memory status", "Show index and provider status."],
          ["assistme memory index --force", "Force a full reindex."],
          ['assistme memory search --query "deployment notes"', "Search indexed memory entries."],
          ["assistme memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.assistme.ai/cli/memory")}\n`,
    );
