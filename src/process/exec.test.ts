...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        ASSISTME_BASE_ENV: "base",
        ASSISTME_TO_REMOVE: undefined,
      },
      env: {
        ASSISTME_TEST_ENV: "ok",
      },
    });

    expect(resolved.ASSISTME_BASE_ENV).toBe("base");
    expect(resolved.ASSISTME_TEST_ENV).toBe("ok");
    expect(resolved.ASSISTME_TO_REMOVE).toBeUndefined();
  });
