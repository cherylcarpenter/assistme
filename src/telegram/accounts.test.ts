...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", ASSISTME_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
      const cfg: AssistMeConfig = {
        channels: {
          telegram: { accounts: { work: { botToken: "tok-work" } } },
        },
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): AssistMeConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): AssistMeConfig => ({
    channels: {
      telegram: {
