...
import {
  probeAuthenticatedAssistMeRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedAssistMeRelay({
    baseUrl,
    relayAuthHeader: "x-assistme-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated assistme relay probe responses", async () => {
