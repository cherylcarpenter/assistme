...
const RELAY_TOKEN_CONTEXT = "assistme-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const ASSISTME_RELAY_BROWSER = "AssistMe/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.ASSISTME_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or ASSISTME_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedAssistMeRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === ASSISTME_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
