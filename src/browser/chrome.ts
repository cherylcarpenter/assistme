...
export function resolveAssistMeUserDataDir(profileName = DEFAULT_ASSISTME_BROWSER_PROFILE_NAME) {
...
export async function launchAssistMeChrome(
  ...
  const userDataDir = resolveAssistMeUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ASSISTME_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateAssistMeProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 assistme browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`assistme browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`assistme browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 assistme browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopAssistMeChrome(
