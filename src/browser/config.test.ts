...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("assistme");
    expect(profile?.driver).toBe("assistme");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const assistme = resolveProfile(resolved, "assistme");
    expect(assistme?.driver).toBe("assistme");
    expect(assistme?.cdpPort).toBe(18800);
    expect(assistme?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from ASSISTME_GATEWAY_PORT when unset", () => {
    withEnv({ ASSISTME_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const assistme = resolveProfile(resolved, "assistme");
      expect(assistme?.cdpPort).toBe(19012);
      expect(assistme?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ ASSISTME_GATEWAY_PORT: undefined }, () => {
      ...
      const assistme = resolveProfile(resolved, "assistme");
      expect(assistme?.cdpPort).toBe(19022);
      expect(assistme?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(assistme?.cdpPort).toBe(19000);
    expect(assistme?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "assistme");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        assistme: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("assistme");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to assistme profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps assistme default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("assistme");
    ...
    it("keeps assistme default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("assistme");
    ...
    it("keeps assistme default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
