  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          assistme: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ASSISTME_GATEWAY_PORT;
    process.env.ASSISTME_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ASSISTME_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ASSISTME_GATEWAY_PASSWORD;
