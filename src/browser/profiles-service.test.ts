...
vi.mock("./chrome.js", () => ({
  resolveAssistMeUserDataDir: vi.fn(() => "/tmp/assistme-test/assistme/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveAssistMeUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "assistme-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveAssistMeUserDataDir).mockReturnValue(userDataDir);
