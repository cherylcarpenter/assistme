...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".assistme-profile-decorated");
}
 ...
 */
export function decorateAssistMeProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ASSISTME_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ASSISTME_BROWSER_COLOR).toUpperCase();
