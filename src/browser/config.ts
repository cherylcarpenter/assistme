import type { BrowserConfig, BrowserProfileConfig, AssistMeConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_ASSISTME_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_ASSISTME_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "assistme" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ASSISTME_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ASSISTME_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an AssistMe browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for assistme/assistme CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AssistMeConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ASSISTME_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_ASSISTME_BROWSER_PROFILE_NAME]
        ? DEFAULT_ASSISTME_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "assistme";
