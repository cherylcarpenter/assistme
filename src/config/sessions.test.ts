...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ASSISTME_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-assistme",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-assistme#general");
  });
  ...
  it("derives session transcripts dir from ASSISTME_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ASSISTME_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ASSISTME_STATE_DIR differs from stored paths", () => {
