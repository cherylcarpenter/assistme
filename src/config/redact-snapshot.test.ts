...
import type { ConfigFileSnapshot } from "./types.assistme.js";
import { AssistMeSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(AssistMeSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.assistme/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/assistme/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/assistme/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/assistme/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/assistme/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = AssistMeSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "AssistMeConfig";
    const hints = mainSchemaHints;
