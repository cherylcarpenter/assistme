...
import type { AssistMeConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareAssistMeVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  assistme config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  assistme config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: AssistMeConfig,
  pathSegments: string[],
): { changed: boolean; next: AssistMeConfig } {
...
function coerceConfig(value: unknown): AssistMeConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AssistMeConfig;
}
...
function stampConfigVersion(cfg: AssistMeConfig): AssistMeConfig {
...
function warnIfConfigFromFuture(cfg: AssistMeConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareAssistMeVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer AssistMe (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AssistMeConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AssistMeConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AssistMeConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AssistMeConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as AssistMeConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ASSISTME_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ASSISTME_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.ASSISTME_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.ASSISTME_WATCH_SESSION === "string" &&
        deps.env.ASSISTME_WATCH_SESSION.trim().length > 0
          ? deps.env.ASSISTME_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.ASSISTME_WATCH_COMMAND === "string" &&
        deps.env.ASSISTME_WATCH_COMMAND.trim().length > 0
          ? deps.env.ASSISTME_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `ASSISTME_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: AssistMeConfig | null = null;
let runtimeConfigSourceSnapshot: AssistMeConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ASSISTME_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ASSISTME_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): AssistMeConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): AssistMeConfig {
