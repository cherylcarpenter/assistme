...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("assistme-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "assistme-sandbox-browser-global" },
      agentBrowser: { network: "assistme-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("assistme-sandbox-browser-agent");
  });
