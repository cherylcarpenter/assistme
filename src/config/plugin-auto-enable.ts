...
function isWhatsAppConfigured(cfg: AssistMeConfig): boolean {
...
function isGenericChannelConfigured(cfg: AssistMeConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: AssistMeConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AssistMeConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: AssistMeConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: AssistMeConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: AssistMeConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: AssistMeConfig, pluginId: string): AssistMeConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
