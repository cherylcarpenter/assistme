...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.assistme/assistme.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".assistme", 19001);
  ...
  it("defaults to ~/.assistme/assistme.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".assistme", "assistme.json"));
    });
  });

  it("uses ASSISTME_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { ASSISTME_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".assistme", "assistme.json"));
    });
  });

  it("honors explicit ASSISTME_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".assistme", 20002, "custom.json");
      const io = createIoForHome(home, { ASSISTME_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("honors legacy CLAWDBOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".assistme", 20003, "legacy-custom.json");
  ...
  it("normalizes safe-bin config entries at config load time", async () => {
    await withTempHome(async (home) => {
      ...
      const configPath = path.join(configDir, "assistme.json");
      await fs.writeFile(
        configPath,
        JSON.stringify(
          {
