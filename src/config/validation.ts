...
import type { AssistMeConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AssistMeConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: AssistMeConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: AssistMeConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AssistMeSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AssistMeConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AssistMeConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as AssistMeConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as AssistMeConfig,
  };
}
