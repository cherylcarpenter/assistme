...
import { buildWebSearchProviderConfig, withTempHome, writeAssistMeConfig } from "./test-helpers.js";
import { AssistMeSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = AssistMeSchema.safeParse({
      $schema: "https://assistme.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://assistme.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = AssistMeSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = AssistMeSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeAssistMeConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeAssistMeConfig(home, {
        gateway: { bind: "${ASSISTME_BIND}" },
      });

      const prev = process.env.ASSISTME_BIND;
      process.env.ASSISTME_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.ASSISTME_BIND;
        } else {
          process.env.ASSISTME_BIND = prev;
        }
      }
    });
  });
