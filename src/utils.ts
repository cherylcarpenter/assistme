...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".assistme");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ASSISTME_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ASSISTME_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ASSISTME_STATE_DIR.
