...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: AssistMeConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: AssistMeConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: AssistMeConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: AssistMeConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: AssistMeConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: AssistMeConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: AssistMeConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
