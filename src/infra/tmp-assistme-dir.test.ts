...
import { POSIX_ASSISTME_TMP_DIR, resolvePreferredAssistMeTmpDir } from "./tmp-assistme-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredAssistMeTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `assistme-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const wrappedLstatSync = vi.fn((target: string) => {
    if (target === POSIX_ASSISTME_TMP_DIR) {
  ...
  const resolved = resolvePreferredAssistMeTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredAssistMeTmpDir", () => {
  it("prefers /tmp/assistme when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ASSISTME_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/assistme when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ASSISTME_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/assistme when /tmp/assistme is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/assistme when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/assistme is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/assistme is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/assistme is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback AssistMe temp dir/);
  });
