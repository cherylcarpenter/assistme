...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ASSISTME_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*assistme-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "assistme";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "assistme");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "assistme-gw");
  ...
  records.push(`_assistme-gw._tcp IN PTR ${instanceLabel}._assistme-gw._tcp`);
  records.push(`${instanceLabel}._assistme-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._assistme-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; assistme-content-hash: ${contentHash}\n${contentBody}`;
}
