...
let ensureAssistMeCliOnPath: typeof import("./path-env.js").ensureAssistMeCliOnPath;

describe("ensureAssistMeCliOnPath", () => {
  const envKeys = [
    "PATH",
    "ASSISTME_PATH_BOOTSTRAPPED",
    "ASSISTME_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureAssistMeCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling assistme exists", () => {
    const tmp = abs("/tmp/assistme-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "assistme");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.ASSISTME_PATH_BOOTSTRAPPED;

    ensureAssistMeCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.ASSISTME_PATH_BOOTSTRAPPED = "1";
    ensureAssistMeCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/assistme-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "assistme");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/assistme-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "assistme");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "assistme");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/assistme-path/case-linuxbrew");
