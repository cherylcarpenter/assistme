...
import { resolvePreferredAssistMeTmpDir } from "../tmp-assistme-dir.js";
...
const telegramChunkConfig: AssistMeConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: AssistMeConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes AssistMe tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredAssistMeTmpDir()]),
      }),
    );
  });

  it("includes AssistMe tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes AssistMe tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes AssistMe tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: AssistMeConfig = { channels: { signal: { mediaMaxMb: 2 } } };
