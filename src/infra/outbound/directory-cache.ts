...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AssistMeConfig | null = null;
  ...
  get(key: string, cfg: AssistMeConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AssistMeConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AssistMeConfig): void {
