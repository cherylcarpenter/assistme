...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred AssistMe tmp root", async () => {
    const tmpRoot = resolvePreferredAssistMeTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideAssistMe = path.join(os.tmpdir(), "outside-assistme", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
