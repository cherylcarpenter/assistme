...
vi.mock("./assistme-root.js", () => ({
  resolveAssistMePackageRoot: vi.fn(async () => null),
  resolveAssistMePackageRootSync: vi.fn(() => null),
}));
...
let assistmeRoot: typeof import("./assistme-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    assistmeRoot = await import("./assistme-root.js");
  });
  ...
  it("uses resolveAssistMePackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/assistme");
    (
      assistmeRoot.resolveAssistMePackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/assistme"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "assistme" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "assistme.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-assistme");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/assistme-bundle");
    (
      assistmeRoot.resolveAssistMePackageRootSync as unknown as ReturnType<typeof vi.fn>
