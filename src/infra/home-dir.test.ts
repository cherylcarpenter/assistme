...
describe("resolveEffectiveHomeDir", () => {
  it("prefers ASSISTME_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/assistme-home"),
    );
  });
  ...
  it("expands ASSISTME_HOME when set to ~", () => {
    const env = {
      ASSISTME_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for ASSISTME_HOME", () => {
    const result = resolveRequiredHomeDir(
      { ASSISTME_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when ASSISTME_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ ASSISTME_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { ASSISTME_HOME: "/srv/assistme-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/assistme-home")}/x`);
  });
