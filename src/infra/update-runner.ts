...
const DEFAULT_PACKAGE_NAME = "assistme";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["assistme", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "assistme.mjs");
    ...
    const doctorStep = await runStep(
      step("assistme doctor", doctorArgv, gitRoot, { ASSISTME_UPDATE_IN_PROGRESS: "1" }),
    );
