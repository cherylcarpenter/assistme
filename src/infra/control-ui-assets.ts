...
import { resolveAssistMePackageRoot, resolveAssistMePackageRootSync } from "./assistme-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAssistMePackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "assistme" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveAssistMePackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // assistme.mjs or dist/<bundle>.js
