...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["ASSISTME_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.ASSISTME_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveAssistMePackageRoot } = await import("./assistme-root.js"));
    ...
    vi.mocked(resolveAssistMePackageRoot).mockClear();
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveAssistMePackageRoot).mockResolvedValue("/opt/assistme");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/assistme/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
