...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ASSISTME_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ASSISTME_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ASSISTME_APNS_TEAM_ID and ASSISTME_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ASSISTME_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ASSISTME_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ASSISTME_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ASSISTME_APNS_PRIVATE_KEY_P8 or ASSISTME_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ASSISTME_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createAssistMePushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    assistme: createAssistMePushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
