...
export function triggerAssistMeRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ASSISTME_SYSTEMD_UNIT,
        process.env.ASSISTME_PROFILE,
      );
  ...
  const label =
    process.env.ASSISTME_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ASSISTME_PROFILE);
