...
function candidateBinDirs(opts: EnsureAssistMePathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `assistme` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "assistme");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ASSISTME_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "assistme"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `assistme` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAssistMeCliOnPath(opts: EnsureAssistMePathOpts = {}) {
  if (isTruthyEnvValue(process.env.ASSISTME_PATH_BOOTSTRAPPED)) {
    return;
  }
