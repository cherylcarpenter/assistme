...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__assistme_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "assistme-root");
...
describe("resolveAssistMePackageRoot", () => {
  let resolveAssistMePackageRoot: typeof import("./assistme-root.js").resolveAssistMePackageRoot;
  let resolveAssistMePackageRootSync: typeof import("./assistme-root.js").resolveAssistMePackageRootSync;

  beforeAll(async () => {
    ({ resolveAssistMePackageRoot, resolveAssistMePackageRootSync } =
      await import("./assistme-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "assistme");
    const pkgRoot = path.join(project, "node_modules", "assistme");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "assistme" }));

    expect(resolveAssistMePackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "assistme");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "assistme.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "assistme" }));

    expect(resolveAssistMePackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveAssistMePackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-assistme package roots", async () => {
    const pkgRoot = fx("not-assistme");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-assistme" }));

    expect(resolveAssistMePackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveAssistMePackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveAssistMePackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
