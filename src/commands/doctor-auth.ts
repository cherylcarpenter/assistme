...
function pruneAuthProfiles(
  cfg: AssistMeConfig,
  profileIds: Set<string>,
): { next: AssistMeConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("assistme models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("assistme models auth setup-token")} or ${formatCliCommand(
      "assistme configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "assistme models auth login --provider openai-codex",
    )} or ${formatCliCommand("assistme configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("assistme configure")}\` or \`${formatCliCommand("assistme onboard")}\`.`;
}
