...
export async function statusCommand(
  ...
  runtime.log(theme.heading("AssistMe status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`assistme devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("assistme devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("assistme devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("assistme security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("assistme security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.assistme.ai/faq");
  runtime.log("Troubleshooting: https://docs.assistme.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("assistme status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("assistme logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("assistme status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("assistme gateway probe")}`);
  }
}
