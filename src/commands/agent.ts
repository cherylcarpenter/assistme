...
function prependInternalEventContext(
  ...
  if (body.includes("AssistMe runtime context (internal):")) {
    return body;
  }
...
async function agentCommandInternal(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("assistme agents list")}" to see configured agents.`,
      );
    }
  }
