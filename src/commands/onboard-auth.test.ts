...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under ASSISTME_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("assistme-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-oauth-sync-"));
    ...
    process.env.ASSISTME_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-oauth-nosync-"));
    process.env.ASSISTME_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ASSISTME_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-oauth-external-"));
    process.env.ASSISTME_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ASSISTME_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ASSISTME_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("assistme-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
