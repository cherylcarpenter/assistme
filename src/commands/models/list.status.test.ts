...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAssistMeAgentDir: vi.fn().mockReturnValue("/tmp/assistme-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/assistme-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/assistme-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveAssistMeAgentDir: mocks.resolveAssistMeAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/assistme-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAssistMeAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/assistme-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/assistme-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/assistme-agent-custom");
