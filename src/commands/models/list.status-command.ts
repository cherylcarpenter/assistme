...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAssistMeAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("assistme models auth setup-token")}\` or \`${formatCliCommand("assistme configure")}\`.`
          : `Run \`${formatCliCommand("assistme configure")}\` or set an API key env var.`;
