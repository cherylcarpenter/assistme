...
export async function loadValidConfigOrThrow(): Promise<AssistMeConfig> {
...
export async function updateConfig(
  mutator: (cfg: AssistMeConfig) => AssistMeConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AssistMeConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AssistMeConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("assistme agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
