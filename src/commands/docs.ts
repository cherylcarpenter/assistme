...
const SEARCH_TOOL = "https://docs.assistme.ai/mcp.SearchAssistMe";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.assistme.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('assistme docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.assistme.ai/");
      runtime.log(`Search: ${formatCliCommand('assistme docs "your query"')}`);
    }
    return;
  }
