...
beforeAll(() => {
  envSnapshot = captureEnv(["ASSISTME_PROFILE"]);
  process.env.ASSISTME_PROFILE = "isolated";
});
...
vi.mock("../infra/assistme-root.js", () => ({
  resolveAssistMePackageRoot: vi.fn().mockResolvedValue("/tmp/assistme"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/assistme/pnpm-lock.yaml",
      markerPath: "/tmp/assistme/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.assistme.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.assistme.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("assistme status --all") ||
          line.includes("assistme --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("ASSISTME_GATEWAY_TOKEN", "abcd1234", async () => {
