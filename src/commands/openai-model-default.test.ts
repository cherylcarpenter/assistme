...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: AssistMeConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: AssistMeConfig) => { changed: boolean; next: AssistMeConfig };
  defaultModel: string;
  overrideConfig: AssistMeConfig;
  alreadyDefaultConfig: AssistMeConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: AssistMeConfig) => config,
      applyDefaultConfig: (config: AssistMeConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: AssistMeConfig = { agents: { defaults: {} } };
