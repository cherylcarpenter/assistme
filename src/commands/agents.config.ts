...
function resolveAgentName(cfg: AssistMeConfig, agentId: string) {
...
function resolveAgentModel(cfg: AssistMeConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AssistMeConfig): AgentSummary[] {
