...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "AssistMe", ".assistme");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/assistme-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".assistme");
  ...
  it("anchors cloud detection to OS homedir when ASSISTME_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".assistme");
    const originalAssistMeHome = process.env.ASSISTME_HOME;
    process.env.ASSISTME_HOME = "/tmp/assistme-home-override";
    ...
    try {
      ...
      if (originalAssistMeHome === undefined) {
        delete process.env.ASSISTME_HOME;
      } else {
        process.env.ASSISTME_HOME = originalAssistMeHome;
      }
    }
  });
