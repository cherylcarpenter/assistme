...
function resolveMode(cfg: AssistMeConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("AssistMe doctor");

  const root = await resolveAssistMePackageRoot({
  ...
  let cfg: AssistMeConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("assistme configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("assistme config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("assistme setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("assistme doctor --fix")}" to apply changes.`);
  }
