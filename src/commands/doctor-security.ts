...
import type { AssistMeConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: AssistMeConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("assistme security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.assistme/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("assistme approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.assistme.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("assistme configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("assistme config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("assistme doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("assistme config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('assistme config set session.dmScope "per-channel-peer"') +
