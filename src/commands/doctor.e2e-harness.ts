...
export const resolveAssistMePackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadAssistMePlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveAssistMePackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ASSISTME_STATE_DIR;
  originalUpdateInProgress = process.env.ASSISTME_UPDATE_IN_PROGRESS;
  process.env.ASSISTME_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "assistme-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ASSISTME_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ASSISTME_UPDATE_IN_PROGRESS;
  } else {
    process.env.ASSISTME_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
