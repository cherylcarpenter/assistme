...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ASSISTME_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ASSISTME_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ASSISTME_GATEWAY_TOKEN");
    expect(message).not.toContain("ASSISTME_GATEWAY_PASSWORD");
  });
