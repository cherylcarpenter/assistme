...
type PromptDefaultModelResult = { model?: string; config?: AssistMeConfig };
...
function resolveConfiguredModelRaw(cfg: AssistMeConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: AssistMeConfig): string[] {
...
export function applyPrimaryModel(cfg: AssistMeConfig, model: string): AssistMeConfig {
...
export function applyModelAllowlist(cfg: AssistMeConfig, models: string[]): AssistMeConfig {
  const defaults = cfg.agents?.defaults;
