...
export function requireAssistMeAgentDir(): string {
  const agentDir = process.env.ASSISTME_AGENT_DIR;
  if (!agentDir) {
    throw new Error("ASSISTME_AGENT_DIR not set");
  }
  return agentDir;
}
