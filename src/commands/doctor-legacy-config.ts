...
export function normalizeCompatibilityConfigValues(cfg: AssistMeConfig): {
  config: AssistMeConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: AssistMeConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as AssistMeConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as AssistMeConfig["browser"],
    };
    changes.push(
