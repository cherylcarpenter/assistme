...
const { createAssistMeTools } = await import("../agents/assistme-tools.js");
...
type SessionSendTool = ReturnType<typeof createAssistMeTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createAssistMeTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ASSISTME_GATEWAY_PORT", "ASSISTME_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.ASSISTME_GATEWAY_PORT = String(gatewayPort);
  process.env.ASSISTME_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("ASSISTME_CONFIG_PATH missing in gateway test environment");
      }
