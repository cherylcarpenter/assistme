...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "ASSISTME_SKIP_BROWSER_CONTROL_SERVER",
  "ASSISTME_SKIP_GMAIL_WATCHER",
  "ASSISTME_SKIP_CANVAS_HOST",
  "ASSISTME_BUNDLED_PLUGINS_DIR",
  "ASSISTME_SKIP_CHANNELS",
  "ASSISTME_SKIP_PROVIDERS",
  "ASSISTME_SKIP_CRON",
  "ASSISTME_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.ASSISTME_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.ASSISTME_SKIP_GMAIL_WATCHER = "1";
  process.env.ASSISTME_SKIP_CANVAS_HOST = "1";
  process.env.ASSISTME_SKIP_CHANNELS = "1";
  process.env.ASSISTME_SKIP_PROVIDERS = "1";
  process.env.ASSISTME_SKIP_CRON = "1";
  process.env.ASSISTME_TEST_MINIMAL_GATEWAY = "1";
  process.env.ASSISTME_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "assistme-test-no-bundled-extensions")
    : "assistme-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".assistme-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".assistme-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__assistmeTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__assistmeTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["ASSISTME_GATEWAY_TOKEN"]);
  const prev = process.env.ASSISTME_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ASSISTME_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.ASSISTME_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
