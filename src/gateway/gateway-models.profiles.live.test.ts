...
import { resolveAssistMeAgentDir } from "../agents/agent-paths.js";
...
import { ensureAssistMeModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, AssistMeConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.ASSISTME_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ASSISTME_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ASSISTME_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ASSISTME_LIVE_GATEWAY_PROVIDERS);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.ASSISTME_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.ASSISTME_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<AssistMeConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.ASSISTME_CONFIG_PATH,
    token: process.env.ASSISTME_GATEWAY_TOKEN,
    skipChannels: process.env.ASSISTME_SKIP_CHANNELS,
    skipGmail: process.env.ASSISTME_SKIP_GMAIL_WATCHER,
    skipCron: process.env.ASSISTME_SKIP_CRON,
    skipCanvas: process.env.ASSISTME_SKIP_CANVAS_HOST,
    agentDir: process.env.ASSISTME_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.ASSISTME_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.ASSISTME_SKIP_CHANNELS = "1";
  process.env.ASSISTME_SKIP_GMAIL_WATCHER = "1";
  process.env.ASSISTME_SKIP_CRON = "1";
  process.env.ASSISTME_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.ASSISTME_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveAssistMeAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-live-state-"));
  process.env.ASSISTME_STATE_DIR = tempStateDir;
  ...
  process.env.ASSISTME_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.assistme-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveAssistMeAgentDir();
  const sanitizedCfg: AssistMeConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-live-"));
  const tempConfigPath = path.join(tempDir, "assistme.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.ASSISTME_CONFIG_PATH = tempConfigPath;

  await ensureAssistMeModelsJson(nextCfg);
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "AssistMe live tool probe (local, safe): " +
    ...
    process.env.ASSISTME_CONFIG_PATH = previous.configPath;
    process.env.ASSISTME_GATEWAY_TOKEN = previous.token;
    process.env.ASSISTME_SKIP_CHANNELS = previous.skipChannels;
    process.env.ASSISTME_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ASSISTME_SKIP_CRON = previous.skipCron;
    process.env.ASSISTME_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ASSISTME_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ASSISTME_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureAssistMeModelsJson(cfg);
      ...
      const rawModels = process.env.ASSISTME_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via ASSISTME_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.assistme-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
