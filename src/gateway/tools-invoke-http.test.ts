...
let lastCreateAssistMeToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/assistme-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateAssistMeToolsContext?.agentTo,
          agentThreadId: lastCreateAssistMeToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createAssistMeTools: (ctx: Record<string, unknown>) => {
      lastCreateAssistMeToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateAssistMeToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-assistme-message-to": "channel:24514",
        "x-assistme-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
