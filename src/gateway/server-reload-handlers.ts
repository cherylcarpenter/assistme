  ...
  const applyHotReload = async (
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (ASSISTME_SKIP_GMAIL_WATCHER=1)"),
      });
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (ASSISTME_SKIP_CHANNELS=1 or ASSISTME_SKIP_PROVIDERS=1)",
        );
      } else {
