...
import { createAssistMeTools } from "../agents/assistme-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-assistme-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-assistme-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-assistme-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-assistme-thread-id")?.trim() || undefined;
  ...
  const allTools = createAssistMeTools({
