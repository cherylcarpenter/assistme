...
export function resolveHooksConfig(cfg: AssistMeConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AssistMeConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-assistme-token"] === "string"
      ? req.headers["x-assistme-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
