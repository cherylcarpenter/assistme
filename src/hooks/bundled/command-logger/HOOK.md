...
homepage: https://docs.assistme.ai/automation/hooks#command-logger
metadata:
  {
    "assistme":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.assistme/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.assistme/logs/commands.log ~/.assistme/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/assistme`:
   ```
   /home/username/.assistme/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
