...
This directory contains hooks that ship with AssistMe. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.assistme/assistme.json`:
...
2. Restart gateway: `pkill -9 -f 'assistme.*gateway' && pnpm assistme gateway`
3. Enable the hook: `assistme hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.assistme.ai/automation/hooks
