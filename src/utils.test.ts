...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("assistme-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("assistme-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("assistme-lid-a-", (first) => {
      withTempDirSync("assistme-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.assistme when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "assistme-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers ASSISTME_HOME over HOME", () => {
    vi.stubEnv("ASSISTME_HOME", "/srv/assistme-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/assistme-home"));
...
describe("shortenHomePath", () => {
  it("uses $ASSISTME_HOME prefix when ASSISTME_HOME is set", () => {
    vi.stubEnv("ASSISTME_HOME", "/srv/assistme-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/assistme-home")}/.assistme/assistme.json`)).toBe(
      "$ASSISTME_HOME/.assistme/assistme.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $ASSISTME_HOME replacement when ASSISTME_HOME is set", () => {
    vi.stubEnv("ASSISTME_HOME", "/srv/assistme-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/assistme-home")}/.assistme/assistme.json`),
    ).toBe("config: $ASSISTME_HOME/.assistme/assistme.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/assistme")).toBe(path.resolve(os.homedir(), "assistme"));
  });
  ...
  it("prefers ASSISTME_HOME for tilde expansion", () => {
    vi.stubEnv("ASSISTME_HOME", "/srv/assistme-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/assistme")).toBe(path.resolve("/srv/assistme-home", "assistme"));
