...
import { __testing, loadAssistMePlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "assistme-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.ASSISTME_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "assistme.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.ASSISTME_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadAssistMePlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          assistme: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.ASSISTME_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.ASSISTME_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadAssistMePlugins>) {
...
function useNoBundledPlugins() {
  process.env.ASSISTME_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadAssistMePlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.ASSISTME_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.ASSISTME_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadAssistMePlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadAssistMePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadAssistMePlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadAssistMePlugins(options);
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ ASSISTME_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
