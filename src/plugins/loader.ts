...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AssistMePluginDefinition;
  register?: AssistMePluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AssistMePluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AssistMePluginDefinition;
...
export function loadAssistMePlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAssistMePlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "assistme/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "assistme/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AssistMePluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as AssistMePluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
