...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@assistme/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "assistme-catalog-"));
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: AssistMeConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
