...
import { discoverAssistMePlugins } from "../../plugins/discovery.js";
import type { AssistMePackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, AssistMePackageManifest>>;
...
const ENV_CATALOG_PATHS = ["ASSISTME_PLUGIN_CATALOG_PATHS", "ASSISTME_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<AssistMePackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: AssistMePackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: AssistMePackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverAssistMePlugins({ workspaceDir: options.workspaceDir });
