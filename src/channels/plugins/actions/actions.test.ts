...
function telegramCfg(): AssistMeConfig {
  return { channels: { telegram: { botToken: "tok" } } } as AssistMeConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: AssistMeConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as AssistMeConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as AssistMeConfig;
...
function createSignalAccountOverrideCfg(): AssistMeConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as AssistMeConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as AssistMeConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as AssistMeConfig,
        accountId: undefined,
        params: {
