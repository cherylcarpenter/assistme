...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: AssistMeConfig, policy: ChannelAccessPolicy) => AssistMeConfig;
  resolveAllowlist: (params: { cfg: AssistMeConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: AssistMeConfig; resolved: TResolved }) => AssistMeConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: AssistMeConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: AssistMeConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: AssistMeConfig, policy: ChannelAccessPolicy): AssistMeConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: AssistMeConfig, policy: ChannelAccessPolicy): AssistMeConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: AssistMeConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: AssistMeConfig; resolved: string[] }) => {
      calls.push("apply");
