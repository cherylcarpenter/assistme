...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ASSISTME_GATEWAY_TOKEN", "ASSISTME_GATEWAY_PASSWORD"]);
  ...
  it("uses ASSISTME_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ ASSISTME_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers ASSISTME_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ ASSISTME_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
