...
const { makeStorePath } = createCronStoreHarness({ prefix: "assistme-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ASSISTME_HOME for tilde expansion", () => {
    vi.stubEnv("ASSISTME_HOME", "/srv/assistme-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/assistme-home", "cron", "jobs.json"));
  });
});
