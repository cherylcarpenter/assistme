...
export type RunCronAgentTurnResult = {
   ...
   * messages.  See: https://github.com/assistme/assistme/issues/15692
   */
  delivered?: boolean;
  /**
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.ASSISTME_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.assistme/agents/<agentId>/agent/).
  ...
  const cfgWithAgentDefaults: AssistMeConfig = {
    ...params.cfg,
    ...
    const fallbackResult = await runWithModelFallback({
      ...
      run: (providerOverride, modelOverride) => {
        ...
        if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
          ...
          // the configured timeoutSeconds. See: https://github.com/assistme/assistme/issues/29774
