...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "assistme-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".assistme", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AssistMeConfig> = {},
): AssistMeConfig {
  const base: AssistMeConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<AssistMeConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<AssistMeConfig>,
