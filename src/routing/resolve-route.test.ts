...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: AssistMeConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: AssistMeConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/assistme-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<AssistMeConfig["bindings"]>[number];
