...
function listAgents(cfg: AssistMeConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: AssistMeConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<AssistMeConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: AssistMeConfig): AgentLookupCache {
...
function pickFirstExistingAgentId(cfg: AssistMeConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AssistMeConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
  byChannelAccountIndex: Map<string, EvaluatedBindingsIndex>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<AssistMeConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  AssistMeConfig,
  {
    bindingsRef: AssistMeConfig["bindings"];
    agentsRef: AssistMeConfig["agents"];
    sessionRef: AssistMeConfig["session"];
...
function resolveRouteCacheForConfig(cfg: AssistMeConfig): Map<string, ResolvedAgentRoute> {
