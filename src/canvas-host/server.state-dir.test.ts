...
describe("canvas host state dir defaults", () => {
  it("uses ASSISTME_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("assistme-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("AssistMe Canvas");
      } finally {
        await handler.close();
      }
    });
