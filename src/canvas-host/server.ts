...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.assistmeCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.assistmeCanvasA2UIAction &&
        typeof window.assistmeCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.assistmeSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("assistme:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android AssistMe node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.assistmeSendUserAction === "function"
        ? window.assistmeSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ASSISTME_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.ASSISTME_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>AssistMe Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
