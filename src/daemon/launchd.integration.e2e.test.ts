...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `assistme-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      ASSISTME_LAUNCHD_LABEL: `ai.assistme.launchd-int-${testId}`,
      ASSISTME_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
