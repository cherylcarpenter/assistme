...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ASSISTME_GATEWAY_PORT).toBe("18789");
    expect(env.ASSISTME_GATEWAY_TOKEN).toBe("secret");
    expect(env.ASSISTME_SERVICE_MARKER).toBe("assistme");
    expect(env.ASSISTME_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ASSISTME_SERVICE_VERSION).toBe("string");
    expect(env.ASSISTME_SYSTEMD_UNIT).toBe("assistme-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ASSISTME_LAUNCHD_LABEL).toBe("ai.assistme.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ASSISTME_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ASSISTME_SYSTEMD_UNIT).toBe("assistme-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ASSISTME_LAUNCHD_LABEL).toBe("ai.assistme.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through ASSISTME_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ASSISTME_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.ASSISTME_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to ASSISTME_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.ASSISTME_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers ASSISTME_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        ASSISTME_GATEWAY_TOKEN: "assistme-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.ASSISTME_GATEWAY_TOKEN).toBe("assistme-token");
  });

  it("omits ASSISTME_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.ASSISTME_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".assistme"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ASSISTME_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".assistme-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ASSISTME_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".assistme"));
  });

  it("uses ASSISTME_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ASSISTME_STATE_DIR: "/var/lib/assistme" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/assistme"));
  });

  it("expands ~ in ASSISTME_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ASSISTME_STATE_DIR: "~/assistme-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/assistme-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ASSISTME_STATE_DIR: "C:\\State\\assistme" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\assistme");
  });
});
