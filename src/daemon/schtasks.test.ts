...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when ASSISTME_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".assistme", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when ASSISTME_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", ASSISTME_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".assistme-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers ASSISTME_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        ASSISTME_PROFILE: "rescue",
        ASSISTME_STATE_DIR: "C:\\State\\assistme",
      },
      expected: path.join("C:\\State\\assistme", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", ASSISTME_PROFILE: "default" },
      expected: path.join("/home/test", ".assistme", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-schtasks-test-"));
    try {
      const extraEnv = typeof options.env === "function" ? options.env(tmpDir) : options.env;
      const env = {
        USERPROFILE: tmpDir,
        ASSISTME_PROFILE: "default",
        ...extraEnv,
      };
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem AssistMe Gateway",
          "cd /d C:\\Projects\\assistme",
          "set NODE_ENV=production",
          "set ASSISTME_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\assistme",
          environment: {
            NODE_ENV: "production",
            ASSISTME_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\assistme\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\assistme\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\AssistMe Share\\node.exe" "\\\\fileserver\\AssistMe Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\AssistMe Share\\node.exe",
            "\\\\fileserver\\AssistMe Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from ASSISTME_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ ASSISTME_STATE_DIR: path.join(tmpDir, "custom-state") }),
