...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.ASSISTME_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service ASSISTME_GATEWAY_TOKEN does not match gateway.auth.token in assistme.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `assistme gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
