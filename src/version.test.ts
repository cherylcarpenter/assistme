...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "assistme-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "assistme", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "assistme", version: "2.3.4" });
  ...
  it("ignores non-assistme package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers ASSISTME_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        ASSISTME_VERSION: "9.9.9",
        ASSISTME_SERVICE_VERSION: "2.2.2",
  ...
  it("uses service and package fallbacks and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        ASSISTME_VERSION: "   ",
        ASSISTME_SERVICE_VERSION: "  2.0.0  ",
