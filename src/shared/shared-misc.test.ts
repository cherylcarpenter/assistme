...
describe("shared/frontmatter", () => {
  ...
  test("resolveAssistMeManifestBlock parses JSON5 metadata and picks assistme block", () => {
    const frontmatter = {
      metadata: "{ assistme: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveAssistMeManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveAssistMeManifestBlock returns undefined for invalid input", () => {
    expect(resolveAssistMeManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveAssistMeManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveAssistMeManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
