...
const TEMP_MEDIA_PREFIX = "assistme-media-";
...
function createGroqAudioConfig(): AssistMeConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): AssistMeConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): AssistMeConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: AssistMeConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        ASSISTME_AGENT_DIR: isolatedAgentDir,
