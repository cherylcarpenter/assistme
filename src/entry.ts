...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "assistme.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "assistme.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.ASSISTME_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.ASSISTME_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.ASSISTME_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[assistme] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    import("./version.js")
      ...
      .catch((error) => {
        console.error(
          "[assistme] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[assistme] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[assistme] ${parsed.error}`);
      process.exit(2);
    }
