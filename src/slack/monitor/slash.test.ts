...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "assistme_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "assistme_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("assistme_cmdarg")).toBe(true);
    expect(testHarness.options.has("assistme_cmdarg")).toBe(true);
  ...
  it("falls back to static menus when app.options() throws during registration", async () => {
    ...
    expect(actions.has("assistme_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("assistme_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("assistme_cmdarg");
    expect(blockId).toContain("assistme_cmdarg_ext:");
    const token = (blockId ?? "").slice("assistme_cmdarg_ext:".length);
