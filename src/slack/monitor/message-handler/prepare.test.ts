...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "assistme-slack-thread-"));
  });
  ...
  function createThreadSlackCtx(params: { cfg: AssistMeConfig; replies: unknown }) {
...
describe("prepareSlackMessage sender prefix", () => {
  ...
  it("prefixes channel bodies with sender label", async () => {
    const ctx = createSenderPrefixCtx({
      channels: {},
      slashCommand: { command: "/assistme", enabled: true },
    });
